...
#' days. 
#' 
#' @format A data frame with 175058 observations on the following 4 variables.
#' \itemize{
#'    \item date = a date (Date object)
#'    \item currency = a character containing \code{USD} \code{EUR} \code{JPY}
#'    \item expiry = a character containing \code{1M} \code{2M} \code{3M} \code{6M} \code{9M} 
#'    \code{1Y} \code{2Y} \code{3Y} \code{4Y} \code{5Y} \code{6Y} \code{7Y} \code{8Y} 
#'    \code{9Y} \code{10Y} \code{12Y} \code{15Y} \code{20Y} \code{30Y}
#'    \item rate = a numeric vector. The LIBOR rate.
#' }
#' @details 
...
#' rate on the \code{date} listed in the same row as the \code{rate}. For example, in the 
#' first row, the \code{rate} is 0.001550; the \code{date} in that row is 2014-08-21. But 
...
#' who are yet unfamiliar with CDS. We design \code{rates.RData} in this way because it is 
#' easier for other functions in the \code{CDS} package to use this data frame for 
#' calculation.
#' 
#' \code{rates.RData} covers holidays, weekend and business days. As is set by ISDA Standard 
...
#'
#' Also, please notice that in \code{rates.RData}, a currency's type of expiries generally 
#' stays the same along the time, but not always. For example, we check the expiry type of 
#' USD in \code{rates.RData}: for 2004-01-01, there are two types of expirty: 1M, 3Y; for 
...
#' currency in a short time. For example, some dates in \code{rates.RData} do not have a 
#' \code{expiry} of 3Y for \code{USD}. This is not likely to be caused by data error, 
#' since all these data are got from Markit and FRED. Users, however, should be aware of that 
#' some data seem "missing".
#'
#' @source \url{https://www.markit.com/} \url{http://research.stlouisfed.org/fred2/}
#'
#' @examples 
#' data(rates)
#' 
...
#' @seealso \code{\link{download.FRED}} \code{\link{download.markit}} \code{\link{build_rates}}
NULL
#> NULL