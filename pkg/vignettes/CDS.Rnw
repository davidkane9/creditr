...
\caption{
This figure is the Calculation section cropped from Figure 2 It shows calculation results from information in Deal Section. There are several familiar terms, such as Price, Principle, Cash Amount.}
...
We will explain \textbf{discount} and \textbf{survival probability} later in Market section.
...
\textbf{Rec Risk (1\%)} stands for "Recovery Risk 1\%." It measures the change of upfront payment if the recovery risk increases by 1\%.
