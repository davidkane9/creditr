...
If the probability of default changes from year one to year two, then this can make our calculation of the upfront payment a bit more complex. If, for example, the probability of default during year one is 1\%, and the probability of default during year two is 2\%, then the expected payment from J.P. Morgan to the portfolio manager would be ()
