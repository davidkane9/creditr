...
Default Exposure is to the expoure to the default of a CDS contract. It can be calculated with Recovery Rate, Notional Amount, and Principal:
...
\caption{
This figure is the Market section cropped from Figure 2. It contains much information about the interest rate curve we use to price CDS. We will discuss Curve Date, points upfront, default probability, and term structure. In addition, we will introduce some concepts that are not on the screenshot, such as hazard rates, discount factor, and CDS indices.
}
\label{AlcoaIncCDSMarket}
\end{figure}

To understand Market section, we first have to know why we want to take interest rates into consideration when we are pricing a CDS. We have learned the discount factor in an one-period simple case, and we will apply that concept to this real case. Therefore, before introducing any terms on the screenshot, let's first understand \textbf{Discount Factor} in a real-life N-period case.
...
\end{equation}
where $r$ is the spread, $t$ is the time to maturity, and $R$ is the recovery rate. Since we now know the calculation of default probability, we also should familiarize ourselves with the actions in an event of default.
