...
A \textbf{Credit Default Swap} (CDS) is type of credit derivative that has become increasingly popular for the past two decades among banks, security houses, hedge funds, insurance companies, corporates, mutual funds, and pension funds; in other words, a CDS is a tool used by many different companies and institutions. To exemplify the advantages and mechanics of this economic tool, consider the following scenario:

Let's say that there exists a portfolio manager who possesses a certain amount of \textbf{assets}, or money, that he can invest. This portfolio manager decides to buy a \textbf{bond}--an agreement in which one party lends money to another party in exchange for that same sum of money, plus interest, at a future date--from Company A with a one-year \textbf{ maturity date}. So, in five years, Company A is expected to return the same sum of money that the portfolio manager lent. Company A also agrees to pay monthly \textbf{interest} to the portfolio manager on fixed dates described in the contract that both parties signed. However, six months after buying the bond, the manager's confidence in the credit worthiness of Company A begins to waiver--in other words, the manager is worried that the company will \textbf{default} on its bonds (become unable to repay its debts to its bondholders) and that he will therefore lose the money he lent. His concern could be due to a number of reasons: perhaps the company's \textbf{credit rating}, as defined by rating agencies, such as S&P or Moody's, recently decreased, or perhaps the company just recently merged. Either way, the manager wants to feel more secure about his investment. 

While the portfolio manager could just sell his bond and do away with worrying about the bond's credit risk, he is reluctant to do so because that transcation is too expensive--or maybe just too cumbersome. 

Here is where a CDS comes in. 
...
When the hedge fund agrees to this agreement, or when any \textbf{protection seller} offers to bear credit risk for a fee, that protection seller probably believes that the company or institution who borrowed money is somewhat credit-worthy, and will probably not go bankrupt and default. In our case, the hedge fund probably disagrees with the portfolio manager, and believes that company A will not default--otherwise (if the hedge fund believed that company A \emph{will} default), there would be no advantage to striking this kind of deal, as the hedge would more likely have a net loss of money than not! If company A does not default, the hedge fund can walk away with the money from the periodic fees and doesn't have to pay anything in return. 
...
CDSs originated in the late 1980s and was popularized by a team at
