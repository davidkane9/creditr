...
CDSs originated in the late 1980s and were popularized by a team at J.P. Morgan including Blythe Masters \citep{cdsOrigins, blythe}. The CDS market started to develop soon afterwards as banks used CDS contracts to hedge their credit exposures on balance sheets. Many different types of CDSs have since emerged including basket default swaps (BDSs), index CDSs, credit-linked notes, et cetera \citep{jk}. In the \textbf{CDS} package, we focus on calculations related to single-name, or corporate, credit default swaps.
...
To understand the benefits of CDSs as hedging tools, consider the following example: let's say that a pension fund portfolio manager has to allocate her clients' money (a total sum of \$100 million) so that these investments earn a steady return without putting them at a large risk. Therefore, this portfolio manager decides to buy several bonds from a company that has a low credit risk---say, Alcoa Inc. A \textbf{bond} is an agreement in which one party lends money to another party in exchange for that same sum of money, plus interest, at a future date. In the case of a company's default, bondholders are compensated before the company's shareholders. Bonds can be further categorized as "senior" and "junior". As the term suggests, senior bonds usually have the greatest seniority in the issuer's capital structure and are the first to be repaid in the case of default. 

Since our portfolio manager is risk-averse, she chooses to buy senior debt from Alcoa Inc. because she believes it to have a sound business model and a low probability of default. In other words, she considers Alcoa Inc. to be creditworthy. Such low-risk bonds are classified as \textbf{Investment Grade (IG)}. The creditworthiness of a company is often represented by the \textbf{credit rating} determined by rating agencies such as Standard \& Poor's (S\&P) or Moody's. We will not go into the details of how these ratings are determined; we will only note that an Investment Grade bond has a rating of BBB- or higher from S\&P or Baa3 or higher from Moody's. Let's say that our portfolio manager's bonds have a maturity of five years and a coupon rate of (TBD). Alcoa Inc. has a credit rating of (TBD) from S\&P.
...
However, six months after the manager bought the bonds, her confidence in the creditworthiness of Alcoa Inc. begins to waiver. In other words, the manager is worried about some \textbf{credit event} related to the company. A credit event can signify the risk of a company's bankruptcy and resulting default, but the term can be expanded to include failure to pay out debt within a certain amount of time, a substantial decrease in credit rating and the confiscation of assets, among other events. The company's credit rating or some other indicator of the company's credit risk may incentivize the manager to hedge her position and reduce her portfolio's exposure. 

Of course, she could simply sell the bonds; however, that may be premature or even unprofitable. Instead, she could purchase a financial product that has a value that rises with a drop in the bonds' intrinsic value. In other words, she could take a position that allows her to profit from increased credit risk. 
...
CDSs were invented in order to resolve the issues that accompany hedging against risk (as described in \ref{Why CDSs?}). 
...
Although the cash flows involved in a CDS seem pretty simple in figure \ref{cashflow}, the underlying mechanics of a CDS are a bit more complex. To understand this, let us look at the actual CDS data of Alcoa Inc. from Bloomberg\footnote{The two most commonly used sources for CDS related data are Bloomberg and Markit, which we will discuss extensively in a later section.}. Figure \ref{CDSalcoa} below displays many different variables such as RED code, Reference Entity et al. on the CDS of Alcoa Inc. for June 02, 2014.   
...
Some of these variables seem rather straighforward. \textbf{Reference Entity} refers to the name of the company from which the protection buyer in the CDS contract has purchased a bond. In our example, the Reference Entity is Alcoa Inc. The \textbf{RED (Reference Entity Database) Code} is an alphanumeric code that refers to a specific company. Each entity/seniority pair has a unique 6 digit RED
...
This is a glossary for CDS terminology that is necessary for determining the pricing and cash flows associated with CDSs\footnote{Some of the
