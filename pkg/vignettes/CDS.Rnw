...
In order to fully understand the nature of a CDS, we should look at what happens to the simple one-period CDS over the life of its contract. Specifically, consider what happens to risk-neutral probability of default (or risk-neutral "P"), the \textbf{mark-to-market} value, the \textbf{spread}, and the \textbf{profits and losses} (PNL) of both the Citadel portfolio manager and JPM from January 1 to December 31. Assume that Alcoa does not default during that year.

We can think of the mark-to-market value of a CDS contract as what a prospective buyer would pay to receive the CDS protection for the rest of the contract. For example, on June 30, midway through the year, a prospective buyer would pay \$500 to the portfolio manager to replace her as the protection buyer. Since the portfolio manager pays the \$1,000 coupon payment on Janaury 1 and recieves \$500 from the prospective buyer on June 30, both the portfolio manager and prospective buyer pay a net amount of \$500---which makes sense since one year of coverage merits a coupon payment of \$1,000 and thus six months of coverage merits \$500. 

We can consider the spread to be a value equal to the coupon in this particular CDS. While we talk about coupons in terms of percentages---in this case, 1\%---spreads are measured in basis points (bps). For example, 1\% is equal to 100 bps.

PNL represents the change in market value of the CDS contract on, in this case, a day to day basis. A good way of thinking about the PNL of, for example, JPM in this CDS is to consider the fraction of the coupon payment that it \emph{earns} each day. We know that JPM recieves a coupon payment of \$1,000 on January 1, but PNL instead considers what JPM earns each day that it provides protection during that year.

Here is a table that considers what happens to the above-mentioned variables as the contract matures from January 1 to December 31.
...
In our simple one-period case, we say that the coupon is 1\% and the spread is 100 bps. In Table \ref{table.1}, we observe that the spread decreases from 100 bps to 0 bps---a reflection of the fact that, at the end of the contract, JPM has earned the full \$1,000 coupon payment by providing a full year of coverage. This process---the spread drop over the duration of the contract---is known as \textbf{rolling down the curve}. 
...
3. If $D_1$ is 2, and $D_1$ is 28 (not in a leap year) or 29, assume that $D_1$ is 30.

For example, according to this convention, if the simple one-period CDS agreement was initiated on February 28 (during a non-leap year), then we would calculate the number of days between 01/01/$Y_a$ and 02/28/$Y_a$ (where $Y_a$ is some year) to be 59, since we consider February 28 to be the last day in a 30-day month (according to expcetion 3 above):
...
If a CDS agreement was initiated on October 31, and it was a leap year, we would calculate the number of days in between 01/01/$Y_a$ and 10/31/$Y_a$ (where $Y_a$ is some year) to be 300:
