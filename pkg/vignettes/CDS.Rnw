...
\begin{equation}
 \begin{aligned}
  {\rm -\hspace{3pt}C} + \frac{P * V}{(1 + i)} = C - \frac{P * V}{(1 + i)} 
    \end{aligned}
...
\begin{equation}
 \begin{aligned}
  {\rm -\hspace{3pt}\$1,000 + \frac{P * \$100,000}{(1 + 0.1)} = \$1,000} - \frac{P * \$100,000}{(1 + 0.1)} 
    \end{aligned}
\end{equation}
...
\begin{align}
  \rm Portfolio\hspace{3pt}manager's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows &= \rm JPM's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows \\
  \rm Premium\hspace{3pt}leg &= \rm Protection\hspace{3pt}leg \\
  \rm U -\hspace{3pt}C + \frac{P * V * (1 - RR)}{(1 + i)}  &= \rm -\hspace{3pt}U + C - \frac{P * V * (1 - RR)}{(1 + i)}
\end{align}
...
$$U - 1000 + \frac{1.5\%*100000*(1 - 0.55)}{(1 + 0.1)} = - U + 1000 - \frac{1.5\%*100000*(1 - 0.55)}{(1 + 0.1)}$$
...
%% Now, do everything that we did in the two period case, but with the algebra expanded to 5 years. Note that we are not using any Bloomberg screen shots in this section, only algebra. But we are using the same numbers as will appear when we show the Bloomberg screen shot in the next section.

%% At this point, we 
%% a) Reference the two period equation from above. This is the central mathematical trick in CDS pricing: premium leg must equal protection leg. We can't remind the reader of this too often. 
%% b) Create a 20 period version of this equation. Do this carefully, paying close attention to all the relevant subscripts. The probability of default is different in each period, as is the interest rate.
%% (Miller: ) Also have to add the case of non-constant interest rate to two-period case, since it doesn't have that complication now.
%% c) Simplify the formula but making the assumptions that the ISDA standard model makes about constant proprobility of default and agree discount rates derived from the ISDA rates.
%% d) Complexify the model a bit (or at least mention the complexities) to handle things like not having to pay the coupon after a credit event.

%% Note the main trick here is to use June 20 as the start date. That means no accruals and other stuff is much simpler too. I think that this is a good simplification which we will relax when we look at the Bloomberg screen shot.

%% newpage command to make a new page; otherwise the equation looks ugly because of the Bloomberg picture following it

\newpage

\section{Bloomberg and Markit}

In this section, we will start to combine the terms we covered in previous sections with real software. Although we have known most of the basic concepts and terms, there are still many more complications in real CDS trading. The software we choose to demonstrate here are Bloomberg and Markit CDS Calculator. During the development of "CDS" R package, we have also used these two software as referrences. So first, let's take a look at Bloomberg.

\subsection{Bloomberg Overview}
Now let's take a look at a real CDS trading on Bloomberg. The figure below shows the details of an actual CDS contract on Alcoa priced on June 23, 2014.

\begin{figure}[H]
\centering
\includegraphics[width=6.3in, height=3.8in]{images/AlcoaIncCDS.jpg}
\caption{This is a Bloomberg Screenshot that shows a CDS transaction for Alcoa. For clarity, we cropped out some of the information, including the commands and settings. Here, we can see that there are three sections on the screen: the Deal section, Calculator section, and Market section. The Deal section contains the details of the CDS trade, including the trade date, entity, spread, coupon, and maturity, etc. The Calculator section contains the calculation results that are generated based upon the details in Deal section, such as Price, Principal, Upfront, and Spread DV01, etc. The Market section contains information about interest rates, probability of default calculated with spread, and the term structure. Below, we will examine each section.}
\label{AlcoaIncCDS}
\end{figure}
...
\caption{This is a figure of Deal section cropped from Figure 2. Some unfamiliar names are actually the same concepts as we have introduced before. In Deal section, information can be divided into two parts: some are important to CDS pricing calculation, while some are not. We will first introduce the non-calculation part, then the calculation part. Also, users may not need to care about some entries in the non-calculation part if they just focus on transactions in certain regions, say, the U.S. Others, who do international trading, may have to care about almost all information. The Calculation section and Market section, which we will talk about later, will reply heavily on the information specified here.}
\label{deal}
\end{figure}

Let's first focus on some non-calculation parts in the Deal section. \textbf{REF Entity} (Reference Entity) refers to the name of the company (Alcoa, in this case) and the \textbf{RED Pair Code} is a Markit product that stands for Reference Entity Database. Each entity/seniority pair has a unique six-digit RED Pair Code that matches the first six digits of the nine-digit RED Pair Code. Each entity also has a "preferred reference obligation," which is the default reference obligation for CDS  trades. A user can input either the six-digit RED Pair Code or the nine-digit RED Pair Code. The input "014B98" is the six-digit RED Pair Code for "Alcoa". The label \textbf{Debt type}, marked as "Senior", refers to the seniority of the debt. The \textbf{REF Obligation} (Reference Obligation) refers to the bond involved in the CDS, and it matches the ISIN. The label \textbf{Restructuring} refers to which term of debt restructuring we agree on: For example, in U.S., traders usually don't accept debt restructuring as CDS payment, and debt restructuring is often considered the same as bankcrupcy. Therefore, when we are doing CDS trading on U.S. firms, we usually specify Restructuring to "NR" which stands for "No Restructuring." In Europe, however, people usually consider certain type of debt restructuring acceptable and not equivalent to bankcrupcy, so people sometimes specify Restructuring to "MMR" which stands for Modified "Modified Restructuring". 

On the bottom-right corner of the screenshot, we have a slot called \textbf{Maturity}. In the first text box, we see "5Y", which is the \textbf{tenor}, or length, of the contract. This means that the buyer is protected for 5 years in this case. Tenor can also be denoted in months, e.g. "5M". The second text box shows the \textbf{maturity date}, or \textbf{end date}, of the contract. This tells us when the protection is over (or after when the buyer is not protected any more). Also note in the upper-right corner that this contract is of type \textbf{SNAC}, or \text{Standard North American Contract}, which is a convention that specifies how North American single-name CDS are supposed to trade. In European markets, CDS belong to the \textbf{STEC} category, or Standard European Contract. 

On the right side of the screenshot, the "Trd Sprd (bp)" is just \textbf{spread}. Here the spread is equal to 160 basis points, but notice that spread can also be denoted in percentage. Below that we can see "Coupon (bp)", which means \textbf{coupon} in basis points, which here is equal to 100 basis points. Notice that here, the protection buyer will have to make an upfront payment to account for the extra 60 basis points paid quarterly (15 bp per quarter) over a period of five years. 

At the bottom-left corner, we can see the default \textbf{Recovery Rate} set to 0.40 and we use \textbf{Curve Recovery Rate} by default. At the bottom-right corner, we can see a bunch of default values, which we call \textbf{trading conventions}. For example, \textbf{Day Convention} ("Day Cnt") denotes what type of convention of dates we follow in this trading. Here, ACT/360 means that we assume there are 360 days in a year. 

Finally, let's focus on those various types of dates. The \textbf{Trade Date} on the left is the date when we trade CDS. Below that is the \textbf{First Accrual Start Date} ("1st Accr Start"), which is the previous \textbf{roll date} adjusted for weekdays before the Trade Date. Then comes the \textbf{First Coupon Payment Date} ("1st Coupon"), which is the date when we pay our first coupon payment to the seller. This is the first roll date adjusted for weekdays after our trade date. Under that is the \textbf{Penultimate Coupon Payment Date} ("Pen Coupon"), which is the second last date when we pay our coupon payment. It is just the weekday-adjusted roll date before the maturity date. Notice that the maturity date must NOT be adjusted, while other dates must be adjusted for businessdays. In other words, maturity dates can be on non-business days, but other dates cannot.
...
\caption{
This figure is the Calculation section cropped from Figure 2, and it contains much important pricing information of the CDS. The CDS Calculator depends heavily on the information provided in Deal section. There are several familiar terms, such as Price, Principle, Cash Amount.}
\label{AlcoaIncCDSCalculator}
\end{figure}

First, in the upper-right corner, we choose to use ISDA Standard Upfront Model to calculate CDS pricing. In the Calculator section, the mark-to-market value or \textbf{Principal} of this contract is \$287458. As is explained earlier, this amount is the sum of the expected present value of each coupon payment. If we account for the accrued amount which the protection buyer does not have to pay, the upfront value is \$286069. The ISDA protocol, since April 2009, specifies that all premium payments, by default, start on the roll date before the Trade Date. So if the Trade Date is June 24, 2014, the \textbf{Accrual Begin Date} is, by default, June 20, 2014. Now if the Accrual Begin Date is 4 days before the Trade Date, the portfolio manager would not want (and is not obligated) to pay interest for the 5 days she has not received protection for. The \textbf{accrued interest} can be calculated using the equation below:
...
Then, let's focus on the bottom-right corner where four interesting calculations are listed, which are Spread DV01, IR DV01, Recovery Risk (1\%), and Default Exposure. However, to understand these four concepts, we have to introduce \textbf{PV01} first.

\subsubsection{PV01}
\label{sec:PV01}

\textbf{PV01} stands for "Present Value 01." The principal payment can also be equated in terms of PV01, which is the present value of a stream of 1 basis points of payments. 
We can use the formula below to calculate the principal amount (clean upfront payment) with PV01:
...
As we can see in the equation above, we need the coupon index, coupon dates, the day count fraction, discount factor for each date, and survival probability until the date of the respective coupon payment. Before we go into the details of how PV01 is used in determining the market price or the risk measures, it is essential to understand the different components used to calculate it. Coupon dates refer to the dates on which coupon payments are made. Day count fraction is the fraction of the day on which the coupon payment is made upon, such as 360 or 365, depending on the day count convention of the CDS. While the first three components of the above equation seem straightforward, the other two - survivial probability and discount factor require more explanation, and we will explain them together with \textbf{Term Structure} in Market section.

Since we now understand PV01, we can proceed to study the four parameteres on the screenshot. We will begin with "Spread DV01".
...
DV01 should always be positive for a protection buyer since he or she is short credit, and a rising spread is a sign of credit deterioration. Starting with PV01 and taking the derivative with respect to the spread gives us:
...
\textbf{Rec Risk (1\%)} literally stands for "Recovery Risk 1\%." It measures the change of upfront payment if the recovery risk increases by 1\%.


\subsubsection{Default Exposure}
\label{sec:DefaultExpo}

Default Exposure refers to the expoure to default of a CDS contract. It can be calculated with Recovery Rate, Notional Amount, and Principal:
...
\caption{
This figure is the Market section cropped from Figure 2. It contains much information about the interest rate curve we use to price CDS. We will discuss Curve Date, points upfront, default probability, and term structure. In addition, we will introduce the concept of hazard rates, which is not on the screenshot.
}
...
On the screenshot, the "prob" cell refers to \textbf{Default Probability}, which is the estimated probability of default for each maturity by a given time. It can be approximated by
\begin{equation}
  \text{Default Prob} \approx \left [1 - exp \left (\frac{rt}{1-R} \right ) \right ], \nonumber
\end{equation}
where $r$ is the spread, $t$ is the time to maturity, and $R$ is the recovery rate. Since we now know the calculation of default probability, we also should familiarize ourselves with the event of default, especially different types of settlements.
...
%% the below introduction is not relevant to what we are discussing in Bloomberg and Markit, so I commented it out (Miller Zhu)
%\subsubsection{Counterparty Risk of Default}
%\label{sec:CounterpartyRisk}

%When a counterparty sells protection on a bond, there is always the probability that, in case of a default, the counterparty may not be able to pay the protection payment to the protection buyer. Or, conversely, perhaps our portfolio manager is unable to make her periodic payments. The risk of either of these eventualities taking place is known as \textbf{counterparty risk}.


Now let's go back to the screenshot and study another important element in the Market section: \textbf{Term Structure}.
...
A CDS term structure is similar to the interest rate curve in that it has the fair spreads of CDS contracts with different maturities. For liquid reference names, these rates are easily available. If the CDS is not liquid, we can use the underlying bond's price to determine the hazard rates. However, if there are only a couple of maturity dates available, we can assume a flat CDS curve. We will not go into the details of how  these curves are derived but simply note that they are used in calculating the present value of the two legs and the upfront payment. With a CDS term structure, we can also calculate hazard rates and risky discount factor.

\subsubsection{Hazard Rates and Survival Probability}
...
\caption{
This is a screenshot of the Markit CDS Calculator which is available online. It calculates prices for the same CDS trading as the Bloomberg screenshot above.  We will not go into each section on the screenshot and explain every details, but we will notice some key terminology differences and distinctive features of Markit. In addition, since Markit Calculator shows us explicitly the swap curves, so we will introduce the concept of Discount Factor and Risky Discount Factor. In the end, we will also mention the concept of CDS indices.
}
\end{figure}

\subsubsection{Terminology Differences}
First, notice that in the upper-left corner, we have a slot called "Trade Level," which means the same as \textbf{spread}. Also notice that unlike Bloomberg, where spread is usually presented in basis points, here in Markit we can actually select whether to present spread in basis points ("bp") or percentages. Then in section "CDS Contract Terms," "Running Cpn" just means \textbf{coupon}. Also, in section "Advanced Terms," there is an e slot called "Holiday Code", where we can select "none" or "JPY" (This is because the Japanese CDS trading follows a different rules of dates, and the main difference is the trading conventions on holidays).

Second, let's move to section "Calculation Results," where several terms are different from Bloomberg's: "Cash Settlement" is the same as \textbf{Cash Amount} in Bloomberg, "Clean Price" is same as \textbf{Price}, the "Accured Amount" in Markit and Bloomberg has different signs (one is positive while the other negative), and "Credit DV01" is same as \textbf{Spread DV01}.

\subsection{Yield Curve Section and Discount Factor}
...
%% the below equation has no difference from the above equation??? So I commented it out (Miller Zhu)
