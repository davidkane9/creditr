...
This paper will walk the reader through the basics of Credit Defaults Swaps (CDS) by beginning with a simplified example of property insurance and progressing to increasingly complicated cases of one-period, two-period, and n-period CDS examples. After a thorough explanation of each case, this paper introduces Markit and Bloomberg as two sources of financial data that can take several CDS attributes (such as those discussed in earlier sections) as inputs to determine CDS pricing. In other worlds, we introduce the reader will to fundamental CDS concepts and then explore how to apply these concepts to pricing in the CDS market. The paper then relates this information to the CDS package and demonstrates how to use it for the purpose of CDS pricing. 
...
This paper aims to explain the mechanics of Credit Default Swaps (CDS), a type of \textbf{credit derivative} that allows finanical institutions to pass on \textbf{credit risk} to investors who accept that risk in exchange for interest. The paper introduces simplied CDS examples in which a portfolio manager passes on risk to a broker from J.P. Morgan. These simplified examples explore a one-period CDS, a two-period CDS, and an n-period CDS and gradually introduce complexities that slowly complicate the simplified examples. In this way, a reader with minimal previous knowledge of CDS can understand fundamental CDS concepts. 

Once the reader understands how CDS works and the basics of CDS pricing, we can look at data from Bloomberg and Markit that applies these concepts to price CDS. Bloomberg and Markit are two sources that investors can use to determine what kinds of CDS agreements they may want to make.

Finally, once the reader has seen how to apply concepts to price CDS in the CDS market, we introduce the CDS package, which helps the user calculate relevant CDS information when trying to price CDS.


%% that allows finanical institutions to pass on \textbf{credit risk} to investors who accept that risk in exchange for interest. To put it simply, credit risk is the possibility that a borrower will \textbf{default} on her loan, or not pay it back. In this paper, we will focus on \textbf{credit default swaps} (CDS), a type of credit derivative that has become quite popular among pension funds, hedge funds, and investment banks, among other financial institutions. We will explain the mechanics of this financial product and show how this \textbf{CDS} package can be used to manipulate and calcuate relevant CDS information. CDS originated in the late 1980s and were popularized by a team at JPM that included Blythe Masters \citep{cdsOrigins, blythe}. 
...
The \textbf{expected cash flows} for the agreement depend on the probability of property damage. Since the homeowner will pay the \$1,000 premium and will potentially receive \$100,000---if the house gets damaged---the homeowner's expected cash flows are as follows. ("P" refers to the homeowner's prediction of probability of property damage.)
...
Since the insurance company will receive the \$1,000 premium and will potentially pay \$100,000---if the house gets damaged---the insurance company's expected cash flows are as follows. ("P" refers to the insurance company's prediction of probability of property damage.)
...
Even though the expected cash flows must be equal in order for the parties to strike a deal, this does not mean that the homeowner and the insurance company conveniently hold the same prediction for the probability of damage. Even if they have reached a deal, the homeowner does not even necessarily know the insurance company's prediction for the probability of damage, nor does the insurance company know that for the homeowner. Instead, since both parties conceded to the deal, we can infer that the parties were willing to assume or compromise on a certain value of "P"---the \textbf{risk-neutral probability of property damage}---even if that value was not the same as their own prediction for the probability of damage. We calculate the risk-neutral probability of damage by equating the expected cash flows in Equations \ref{eqn:homeownerECF} and \ref{eqn:companyECF}, since that is the condition upon which the parties will agree:
...
Clearly, 1\%---the risk neutral probability of property damage---is a very important value for this agreement. We need to note, however, that this value is not necessarily the \emph{true} probability of property damage, nor are each party's prediction for the probability of damage. Consider an extreme example: what if, as the two parties initiate the agreement, an asteriod sets on a course in which it will crash into the house in six months. In such a case, the probability of damage is 100\%. However, neither party has forseen this occurence, so the risk-neutral value of "P" (1\%) and the parties' predictions of "P" (1.5\% and 0.5\%) are certainly not equal to the true value of "P" (100\%).

\subsubsection{Property Insurance Complications}
\label{sec:PIComplications}

Unfortunately, the above insurance purchase---although relatively simple---made several assumptions that excluded real aspects of the insurance market. Some of these aspects are parallel to aspects in the CDS market that we will exclude in all CDS sections, as well. Below we list such assumptions:

1. We assumed that the homeowner and insurance company were risk-neutral when, in reality, homeowners who purchase property insurance are generally risk-adverse. Since they want to avoid risk, they view the process of purchasing insurance as more of a necessity than an option. As such, they are willing to pay more (i.e. a larger coupon payment) than required for the expected cash flows to be equal. Taking our example, in the insurance market the homeowner may have been willing to pay a coupon of 2\% instead of 1\%.

2. We assumed that both parties would only concede to the insurance agreement if the expected cash flows were equal. As assumption 1 demonstrated, an insurance buyer is generally willing to have a smaller expected cash flow than the insurance company. Conversely, insurance companies need to have higher expected cash flows than their clients (insurance buyers) because they aim to \emph{profit} from selling insurance; if they charge as much as they expect to pay, then they can't expect to make any money as a company. 
...
The insurance agreement made assumptions that this paper will, in fact, account for parallel situations regarding CDS. We list them below:

1. The insurance agreement assumed that any damage was damage that would merit the insurance payment of \$100,000. However, what if, for example, the toaster caught on fire and damaged only the toaster? What types of \textbf{events} does the insurance company define as "damage"? These questions highlight the importance of a detailed insurance contract that specifies the terms of the agreement. 

2. The agreement considered only the following two cases: either the entire house was damaged (and the insurance company paid \$100,000), or the house was not damaged (and the insurance company paid \$0). However, there are other likely outcomes to account for. For example, what if only half of the house was damaged? In that case, it would not make sense for the insurance company to pay \$100,000; instead, the company should compensate only for the damaged section of the house by paying \$50,000. 

3. We stated in the beginning of the property insurance explanation that the interest rate was 0\%. Clearly, this is not the case in the insurance market, so we would need to account for that change in our calculations (which we'll discuss in Section \ref{sec:InterestRate}).

4. We largely simplified the insurance agreement by creating a one-year agreement from January 1 to December 31. This, too, does not have to be the case in the insurance market.

5. The insurance agreement had one premium payment of \$1,000. However, there are cases in the insurance market where they are multiple payments on different dates, and we did not account for that possibility in our example.

\subsection{Simple One-Period Case}
\label{sec:OnePeriod}

Instead of buying insurance against property damage, consider buying insurance against a company's inability to meet its obligations. For example, a portfolio manager from the hedge fund, Citadel, believes that the company, Alcoa, isn't likely to return the money that its bondholders have lent it. In other words, she believes that Alcoa will \textbf{default} on its bonds---not pay them back. She enters a one-year CDS agreement with an investment bank---say, J.P. Morgan (JPM)\footnote{For ease of reading, we desginate the buyers of protection in our examples as female and the sellers of protection as male.}---in which she purchases protection of \$100,000, an amount known as the \textbf{notional amount} of the CDS. Note that bondholders can purchase protection on their bonds (i.e. an Alcoa bondholder can enter a CDS in which she purchases protection against Alcoa's default), but this does not have to be the case. In our example, the portfolio manager does not own Alcoa bonds. 
...
Since JPM will receive the \$1,000 premium and will potentially pay \$100,000 if Alcoa defaults, JPM's expected cash flows---which represent the protection leg---are as follows. 
...
Note that the portfolio manager's expected cash flow is the same as that of the homeowner in the property insurance example, and JPM's expected cash flow is the same as that of the insurance company. This shouldn't be too surprising, though, since the numerical figures in both examples are the same.
...
In order to fully understand the nature of a CDS, we should look at what happens to the simple one-period CDS over the life of its contract. Specifically, consider what happend to risk-neutral probability of default (or risk-neutral "P"), the value of the contract, and the profits \& losses (P \& L) of both the Citadel portfolio manager and JPM from January 1 to December 31. We'll first consider a case in which Alcoa does not default during the simple one-period CDS.

We can think of the "value of the contract" as what a prospective protection buyer would pay to receive the CDS protection for the rest of the contract. For example, on June 30, midway through the year, a prospective buyer would pay \$500 to the portfolio manager to replace her as the protection buyer in the CDS agreement. Since the portfolio manager pays the \$1,000 coupon payment on Janaury 1 and recieves \$500 from the prospective buyer on June 30, both the portfolio manager and prospective buyer pay a net amount of \$500---which makes sense since one year of coverage merits a coupon payment of \$1,000 and thus six months of coverage merits \$500.

The P \& L that we are measuring in Table \ref{table.1} is the daily P \& L; that is, the change in market value of the CDS contract. Instead of considering the exact cash flows, we are measuring what, for example, JPM is earning each day that passes. Even though JPM receives the \$1,000 coupon payment on January 1, it \emph{earns} this payment day by day in small increments over the course of the year. 

Here we measure the above mentioned variables as the contract matures:
...
\begin{tabular}{lllllll}
  \hline
Date & Cash flows (Citadel) & Cash flows (JPM) & Risk-neutral "P" & Value of contract & P \& L (Citadel) & P \& L (JPM) \\ 
  \hline
  January 1 & -\$1,000 & \$1,000 & 1\% & \$1,000 & \$0 & \$0 \\ 
  March 31 & \$0 & \$0 & .75\% & \$750 & -\$250 & \$250 \\ 
  June 30 & \$0 & \$0 & .50\% & \$500 & -\$500 & \$500 \\ 
  December 31 & \$0 & \$0 & 0\% & \$0 & -\$1,000 & \$1,000 \\
   \hline
\end{tabular}
}

\caption{This table measures how the cash flows for each party, the risk-neutral value of "P", the value of the contract, and the P \& L for each party change as the simple one-period CDS matures. Since this table reflects the changes in those variables in the case where the reference entity---Alcoa---does not default, the only cash flow is the \$1,000 coupon payment that the Citadel portfolio manager pays to JPM on January 1. The risk-neutral value of "P" decreases from 1\% to 0\% as the contract matures, a process we will discuss in detail in this section. Similarly, the value of the contract decreases from \$1,000 to \$0 from January 1 to December 31. Note that as JPM profits from providing protection coverage, the portfolio manager loses. On January 1, the P \& L of both parties is 0.} 
\label{table.1}
\end{table}

As we can see in Table \ref{table.1}, the risk-neutral "P" decreases from 1\% on January 1 to 0\% on December 31. On June 30, the risk-neutral "P" had dropped to .5\% because only half of a year remained during which Alcoa could default during the contract. This serves as an example of how there is a direct relationship between risk-neutral probability of default and contract duration. Note that we are referring to the risk-neutral probability of default \emph{within} \emph{the} \emph{context} \emph{of} \emph{this} \emph{specific} \emph{CDS}. 

%% Note how the value of the contract decreases as the contract matures. This process---the spread drop over the duration of the contract---is known as \textbf{rolling down the curve}.
...
Similar to the property insurance example, we made several assumptions in our simple one-period CDS case that are not consistent with the CDS market. 

1. Like in the property insurance example, we assumed that both parties are risk-neutral when the protection buyer could very well be risk-adverse. She may be willing to pay a coupon larger than 1\% for the same deal because she strongly desires to have protection on Alcoa.

2. Also like the property insurance example, we assumed that both parties would only agree to the deal if the expected cash flows were equal. This could very well not be the case if, for example, JPM is selling protection in many CDS agreements and needs to make a profit.

2. In the simple one-period CDS, we assumed that the protection leg would only be paid out in the case of default and bankruptcy. However, depending on the particular CDS, the protection leg could be paid out even if the reference entity only defaults and doesn't go bankrupt. In fact, there are several scenarios that can be considered \textbf{credit events}---occurences that merits the payout of the protection leg in a CDS. Besides default and bankruptcy, common credit events include failure to return money to bondholders within a certain amount of time, a credit rating downgrade (explained in Section \ref{sec:RiskBonds}) and the confiscation of assets, among others.

Because we made many assumptions (and failed to address many aspects of a real CDS), we have split up the following complications into four seperate sections create examples adressing interest rate, recovery rate, accrued coupon and upfront payment. 
...
Solving for "P", the risk-neutral probability of default, we get .91\% (rounded to the nearest hundreth). Since both parties agreed to this CDS in an environment where the interest rate is 10\%, we can say that the portfolio manager's prediction for the probability of default is at least .91\%, and JPM's prediction for the probability of default is at most .91\%. 

\subsubsection{Recovery Rate}
\label{sec:RecoveryRate}

Recall that in our property insurance example, we mentioned that some cases---such as a house fire that burns half of a house---may not merit a full compensation payment equal to the notional amount (in that case, \$100,000). Similarly, when a company defaults, an \textbf{auction} occurs in which bondholders can come to sell their bonds, and prospective buyers can come to buy the bonds at, usually, lower prices. The price at which bonds can be sold after default affects the protection leg payments in CDS contracts.

Suppose that Alcoa defaults during the simple-one period CDS contract, and a bond that had a face value of \$100 can now be sold at the auction for a price of \$55. Note for each complication in the simple one-period CDS, we are ignoring the effects of the other complications. For example, we assume that that there is an interest rate of 0\% in this example.
...
So, we need to factor the recovery rate into our calculation of cash flows because it changes the value of the protection leg in the simple one-period case. Since the portfolio manager and JPM will only both agree to this CDS if the expected cash flows are equal, we set the portfolio manager's cash flows (left side of Equation \ref{eqn:RecoveryRate} below) and JPM's cash flows (right side of Equation \ref{eqn:RecoveryRate} below) equal to each other. ("RR" refers to the recovery rate and, as before, "P" is the probability of default, "C" is the coupon payment, and "V" is the notional value.)
...
Therefore, "P", the risk-neutral probability of default, is 1.8\% (rounded to the nearest tenth) in this scenario.
...
Note that we are dividing 90 by 360 instead of 365 in the above calculation. This particular CDS contract can be said to have a 30/360 \textbf{day count convention}, which means that the accrued coupon calculations are based on the assumptions that there are 30 days in a month and 360 days in a year. For example, if we are trying to find the number of days between date one, M_{1}/D_{1}/Y_{1}, and date two, M_{2}/D_{2}/Y_{2}, using the 30/360 convention, we use the following formula with exceptions listed below:

\begin{equation}
 \begin{aligned}
  {\rm Number of days = 360 * (Y_{2} - Y_{1}) + 30 * (M_{2} - M_{1}) + (D_{2} - D_{2})}
    \end{aligned}
\end{equation}

Exceptions:

1. If D_{1} is 31, assume that D_{1} is 30.

2. If D_{2} is 31 and D_{1} is 30 or 31, assume that D_{2} is 30.

3. If M_{1} is 2, and D_{1} is 28 (not in a leap year) or 29, assume that D_{1} is 30.

For example, according to this convention, if the simple one-period CDS agreement was initiated on February 28 (during a non-leap year), then we would calculate the number of days between 01/01/Y_{a} and 02/28/Y_{a} (where Y_{a} is some year) to be 59, since we consider February 28 to be the last day in a 30-day month (according to expcetion 3 above):

\begin{equation}
 \begin{aligned}
  {\rm Number of days = 360 * (0) + 30 * (2 - 1) + (30 - 1) = 59}
    \end{aligned}
\end{equation}

If a CDS agreement was initiated on October 31, and it was a leap year, we would calculate the number of days in between 01/01/Y_{a} and 10/31/Y_{a} (where Y_{a} is some year) to be 300:

\begin{equation}
 \begin{aligned}
  {\rm Number of days = 360 * (0) + 30 * (10 - 1) + (31 - 1) = 300}
    \end{aligned}
\end{equation}
...
Therefore, the risk-neutral probability of default in this CDS is .75\%. Note that this is different than that of the simple one-period CDS (1\%). Note that in Section BLANK, we observed that the risk-neutral probability of default decreases as the contract matures. Similarly, we note here that if the contract duration is decreases by 25\%, so is the risk-neutral probability of default for that contract.

Consider how the accrued coupon changes as the start date of the contract changes:
...
\begin{tabular}{ll}
  \hline
Contract Start Date & Accrued Coupon \\ 
  \hline
  January 1 & \$0\\ 
  March 31 & \$250\\ 
  June 30 & \$500\\ 
...
\subsubsection{Non-standard Coupon}
\label{sec:nonStandardCoupon}

In all of our CDS examples, we have assumed a coupon of 1\%. We have done so because we have been working with the same basic numerical figures: a risk-neutral probability of default of 1\%, or 100 basis points (bps), and a notional amount of \$100,000:
...
In other words, we have been considered cases in which the portfolio manager believes the probability of default to be at least 1\%, and JPM believes the probability of default to be at most 1\%. 
...
If we want to keep the risk-neutral probability of default ("P") at 3\%, and we want to have a coupon of 1\% (so, \$1,000), the upfront payment needs to account for the difference between a \$3,000 coupon payment and a \$1,000 coupon payment; in other words, the upfront payment needs to be \$2,000, from the portfolio manager to JPM.
...
%% Consider how the value of the upfront payment changes as the contract matures: (???)
...
Note that in Equation \ref{eqn:Upfront}, "C" and "V" are known or pre-determined values, since "C" is standardized by the ISDA, and "V" is agreed upon by the protection buyer and seller. As such, "P" and "U" can be considered the unknowns; you need to know the value of "P" to determine the value of "U", and vice versa.
...
The risk-neutral probability of default---the default that the deal implies both parties are willing to compromise on---can be calculated by equating the expected cash flows. Note that we are using Equation \ref{eqn:SimpleOnePeriod}, like in our calculations for the simple one-period CDS. ("C" stands for coupon payment, "P" is the probability of default, and "V" represents the notional value.)
...
Note that the risk-neutral probability of default in the simple one-period CDS was 1\%. The only reason that this probability is different is that the portfolio manager's coupon payment in the two-period case is double that of the one-period case.

Recall that we discussed in Section \ref{sec:OnePeriodOverTime} how the probability of default within the context of a CDS decreases to 0\% as the contract matures. This CDS is no exception. After one year of the contract, the probability of default over the second year is 1\%---not 2\%. As such, the cash flows for the second year of the contract is as follows:
...
In such a case, the cash flows for year one are as follows:
