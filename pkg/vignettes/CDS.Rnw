...
The calculation part in Deal section is very crucial to the whole CDS pricing. Let's start from defining each new terms. On the bottom-right corner, we have a slot called \textbf{Maturity}. In the first text box, we see "5Y", which is the \textbf{tenor}, or length, of the contract. This means that the buyer is protected for 5 years in this case. Tenor can also be denoted in months, e.g. "5M". The second text box shows the \textbf{maturity date}, or \textbf{end date} of the contract. This means when the protection is over, and after when the buyer is not protected any more. Also note in the upper-right corner that this contract is of type \textbf{SNAC}, or \text{Standard North American Contract}, which is a convention that specifies how North American single-name CDS are supposed to trade. In European markets, CDS belong to the \textbf{STEC} category, or Standard European Contract. Next, let's look at the "Trd Sprd (bp)" on the right side, which just means \textbf{spread}. And we easily know that here the spread is equal to 160 basis points (notice that spread can also be denoted in percentage). Below that we can see "Coupon (bp)", which means \textbf{coupon} in basis points, which here is equal to 100 basis points.
