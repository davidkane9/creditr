...
For example, let's say that there is a portfolio manager who owns a bond--a bond that is part of the CDX IG. While the portfolio manager's confidence in the credit worthiness of company is beginning to waiver--perhaps due to a decrease in the credit rating of the comany, or governmental confiscation of assets--he or she is reluctant to sell this bond because that transaction may be too expensive or too cumbersome to carry out. In this case, the manager does not have to choose between carrying the bond's credit risk--the risk that the company will deafult on its bonds--and completing a transcation that would unecessarily cost him or her time or money. The manager can instead opt to buy protection in the form of a Credit Default Swap (CDS), which would allow him or her to transfer the credit risk to a third party for a fee. 

In our example, let's say that this third party is a hedge fund. So, the portfolio manager sends a periodic fee to a hedge fund manager, and in return, the hedge fund manager offers to pay a much larger sum to the portfolio manager in the case of a credit event--in this case, a default on the bond. That way, the portfolio manager can pass on the bond's credit while keeping the bond in his or her possession, and the hedge fund can receive a fee and possibly not have to pay anything in return if the bond does not default. 

If this hedge fund agrees to become a protection seller in this agreement, the fund probably believes that the company from which the portfolio manager buys the bond is actually somewhat credit-worthy, and will probably not default on its bonds for the duration of the CDS. If that happens, the hedge fund can walk away with the money from the periodic fees, and doesn't have to pay anything in return. If the portfolio manager agrees to become the protection buyer in this type of agreement, however, then this manager probably believes that there is a substantial chance that the company will default on its bonds--as the manager will end up with a net loss money otherwise.

Let's assume that this particular bond did default. As such, the hedge fund did end up having to compensate the portfolio manager for his or her loss in assets. The recovery rate is defined as the amount that the company is able to give back to its debt holders after a credit event has happened.
