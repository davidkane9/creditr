...
A CDS is an agreement between two \textbf{counterparties}, or opposing groups, in which the buyer--in this case, the portfolio manager--pays a \textbf{fixed periodic coupon} to the seller in exchange for protection in the case of a \textbf{credit event} associated with the buyer's bond. A credit event is a term that encompasses the risk of a company's bankruptcy and resulting default; depending on the CDS contract, however, a credit event can be expanded to include failure to pay out debt within a certain amount of time, substantial decrease in credit rating, confiscation of assets, and even effects of wars.

So, in this case, the manager does not have to choose between carrying the bond's credit risk and completing a transcation that would unecessarily cost him time or money. Instead, the manager could opt to buy protection in the form of a CDS, which would allow him or her to effectively transfer the credit risk to a third party who is willing to accept that risk for a fee. 

In our example, let's say that this third party is a hedge fund. So, the portfolio manager sends a periodic fee to a hedge fund manager, and in return, the hedge fund manager offers to pay a much larger sum to the portfolio manager in the case of default. That way, the portfolio manager can pass on the bond's credit risk while keeping the bond in his or her possession, and the hedge fund can receive a fee and possibly not have to pay anything--thus making a profit--in return if the bond does not default. 

If this hedge fund agrees to become a \textbf{protection seller} in this agreement, the fund probably believes that the company from which the portfolio manager buys the bond is actually somewhat credit-worthy, and will probably not default on its bonds for the duration of the CDS. If that happens, the hedge fund can walk away with the money from the periodic fees, and doesn't have to pay anything in return. If the portfolio manager agrees to become the \textbf{protection buyer} in this type of agreement, however, then this manager probably believes that there is a substantial chance that the company will default on its bonds--as the manager will end up with a net loss of money otherwise.

Let's assume that this particular bond did default. As such, the hedge fund ended up having to compensate the portfolio manager for his or her loss in assets. The exact amount or type of compensation would have been defined in the original CDS agreement, but for our purposes, let's assume that the compensation is equal to the total value of the bond minus the company's recovery rate. The \textbf{recovery rate} is defined as the amount that the company is able to give back to its debt holders after a credit event has happened. When a company goes into bankrupcty and is forced to default on its debt, whatever is remaining in the company's assets is paid out according to a \textbf{debt waterfall}. For example, a company's debt is paid out before its stock, and both stock and debt can be labeled as "senior" and "junior," with with the former taking priority over the latter.
...
%% underlying bond to the protection seller. 
