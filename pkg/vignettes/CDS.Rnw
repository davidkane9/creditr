...
%% There are much more elegant ways of setting up a series of equations in LaTeX. Please figure them out.
%% And apply them throughout the document. For example, we want alignment on the = signs here.
...
Having understood a simple one and two period pricing exercise, and having reviewed the basics of spreads, upfronts, accruals, coupons and credit events, we can now consider the N-period case. For concreteness, we will start with a five year on-the-run Alcoa CDS. Assume that today is June 20, 2014 and we are pricing an Alcoa CDS with a \$10 million notional value that matures on September 20, 2019. The CDS pays a 100 bps coupon.
...
<<echo = FALSE>>=
spread.to.pd(data.frame(date = Sys.Date(), spread = 160, tenor = 5, recovery = 0.4, currency = "USD"))
@
