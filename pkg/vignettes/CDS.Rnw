...
If we look at the top of Figure \ref{fig:AlcoaBond}, we see that these bonds have a \textbf{coupon} of 5.72\% and a \textbf{price} of \$112.063. But when Alcoa first issued bonds and sold them to investors on April 02, 2007 (\textbf{Announcement Date} in the figure), it was essentially borrowing \$100 (\textbf{Principal} of one bond) with the agreement that it would pay a fixed 2.86\% (half of 5.72\%) semi-annually for a twelve-year period, at the end of which it would also return the Principal of \$100.   
