...
Looking at the top of Figure \ref{fig:AlcoaBond}, we see that these bonds have a \textbf{coupon} of 5.72\% and a \textbf{price} of \$112.063, and a \textbf{spread} of (will fill in later). When a company first issues bonds, [NEED TO FINISH SENTENCE].

The key variables regarding bond pricing are as follows:\par
\noindent
\textbf{Principal}: the sum that will be returned to the investor when the bond reaches its maturity date. For example, if we own a bond of Alcoa Inc. for \$100 on the day it was issued, we will be paid a balloon payment of \$100 when it matures. This is, of course, in addition to the interest payments. \par
\noindent
...
\noindent
\textbf{Maturity}: length of the bond's life. This will tell us how many coupon payments must be made until the bond's maturity.
