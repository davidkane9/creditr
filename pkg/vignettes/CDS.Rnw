...
\caption{
This figure is the ``Calculation'' section cropped from Figure 2. It shows calculation results from information in Deal Section, such as \textbf{price}, \textbf{principle}, and \textbf{cash amount}. There are also measurements that represent the change of principal if one parameter change by a small amount, such as \textbf{spread DV01}, \textbf{IR DV01}, \textbf{Rec Risk 1\%}. ``Calculator'' section is usually the most important section traders look at, because it contains the calculation results and risk measurements.}
