...
\caption{This is a figure of Deal section cropped from Figure 2. There seem to be unfamiliar names, but they are actually the same concepts as we have introduced before. In Deal section, information can be divided into two parts: some are important to CDS pricing calculation, while some are not. We will first introduce the non-calculation part, then the calculation part. Also, users may not need to care about some entries in the non-calculation part if they just focus on transactions in certain regions, say, the U.S. Others, who do international trading, may have to care about almost all information. The Calculation section and Market section, which we will talk about later, will reply heavily on the information specified here.}
\label{deal}
\end{figure}

Let's first focus on some non-calculation parts in the Deal section.\textbf{REF Entity} (Reference Entity) refers to the name of the company (Alcoa, in this case) and the \textbf{RED Pair Code} is a Markit product that stands for Reference Entity Database. Each entity/seniority pair has a unique six-digit RED Pair Code that matches the first six digits of the nine-digit RED Pair Code. Each entity also has a "preferred reference obligation," which is the default reference obligation for CDS  trades. A user can input either the six-digit RED Pair Code or the nine-digit RED Pair Code. The input "014B98" is the six-digit RED Pair Code for "Alcoa". The label \textbf{Debt type}, marked as "Senior", refers to the seniority of the debt. The \textbf{REF Obligation} (Reference Obligation) refers to the bond involved in the CDS, and it matches the ISIN. The label \textbf{Restructuring} refers to which term of debt restructuring we agree on: For example, in U.S., traders usually don't accept debt restructuring as CDS payment, and debt restructuring is often considered the same as bankcrupcy. Therefore, when we are doing CDS trading of U.S. firms, we usually specify Restructuring to "NR" which stands for "No Restructuring." In Europe, however, people usually consider certain type of debt restructuring acceptable and not equivalent to bankcrupcy, so people sometimes specify Restructuring to "MMR" which stands for Modified "Modified Restructuring". 

These information above are the references that we usually don't use when calculating CDS pricing; and we will talk more about calculation parts in Deal section later, such as different types of dates and other parameters. Now, let's take a look at the calculation part in Deal section, starting from defining each new terms. On the bottom-right corner, we have a slot called \textbf{Maturity}. In the first text box, we see "5Y", which is the \textbf{tenor}, or length, of the contract. This means that the buyer is protected for 5 years in this case. Tenor can also be denoted in months, e.g. "5M". The second text box shows the \textbf{maturity date}, or \textbf{end date} of the contract. This means when the protection is over, and after when the buyer is not protected any more. Also note in the upper-right corner that this contract is of type \textbf{SNAC}, or \text{Standard North American Contract}, which is a convention that specifies how North American single-name CDS are supposed to trade. In European markets, CDS belong to the \textbf{STEC} category, or Standard European Contract. Next, let's look at the "Trd Sprd (bp)" on the right side, which just means \textbf{spread}. And we easily know that here the spread is equal to 160 basis points (notice that spread can also be denoted in percentage). Below that we can see "Coupon (bp)", which means \textbf{coupon} in basis points, which here is equal to 100 basis points. (Notice that here, the protection buyer will have to make an upfront payment to account for the extra 60 basis points paid quarterly (15 bp per quarter) over a period of five years.) Then, let's look at bottom-left corner, where we see the default \textbf{Recovery Rate} set to 0.40 and we use \textbf{Curve Recovery Rate} by default. Next, let's look at the bottom-right corner where we see bunch of default values, which we call \textbf{trading conventions}. For example, \textbf{Day Convention} ("Day Cnt") denotes what type of convention of dates we follow in this trading, as we have mentioned in the simple period cases (Here, ACT/360 means that we assume there are 360 days in a year). Finally, after all the jumping around, let's focus on one of the most important parameters of CDS pricing: various dates. The \textbf{Trade Date} on the left obviously means the date when we trade CDS. Below it is the \textbf{First Accrual Start Date} ("1st Accr Start"), which is the previous \textbf{roll date} adjusted for weekdays before the Trade Date. Below it is the \textbf{First Coupon Payment Date} ("1st Coupon"), which means the date when we pay our first coupon payment to the seller. This is the first roll date adjusted for weekdays after our trade date. Below it is the \textbf{Penultimate Coupon Payment Date} ("Pen Coupon"), which is the second last date when we pay our coupon payment. It is just the weekday-adjusted roll date before the maturity date. Notice that the maturity date must NOT be adjusted for weekdays, while other dates must be adjusted for weekdays. In other words, maturity dates can be on non-business days, but other dates cannot.
...
\caption{
This figure is the Calculation section cropped from Figure 2, and it contains much important pricing information of the CDS. The CDS Calculator depends heavily on the information provided in Deal section. Several familiar terms, such as Price, Principle, Cash Amount, and other interesting results.}
...
Then, let's focus on the bottom-right corner where four interesting calculations are listed, which are Spread DV01, IR DV01, Recovery Risk (1\%), and Default Exposure. However, before we begin explaining these four concepts, another concept must be introduced first, so we will start with PV01 first.

\subsubsection{PV01}
\label{sec:PV01}

\textbf{PV01} literally stands for "Present Value 01." The principal payment can also be equated in the terms of PV01, which is the present value of a stream of 1 basis points of payments. It can also be used to calculate the cash flows and risk measures of a CDS. It is sometimes referred to as the \textbf{CDS duration} or \textbf{risky duration}. Analytically, PV01 can be calculated by
