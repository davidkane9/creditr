...
This paper explains the mechanics of credit default swaps (CDS), a type of \textbf{credit derivative} that transfers \textbf{credit risk} from one group of investors to another, in exchange for payment. A CDS allows one party to purchase insurance (called ``protection'') on a certain investment from another party. The paper introduces simplied CDS examples in which a portfolio manager passes on risk to J.P. Morgan in a one-period CDS, a two-period CDS, and an n-period CDS. With each example, we gradually introduce complications that alter pricing calculations. We make several assumptions, some of which we account for in later calculations, and some of which we maintain throughout the paper. A reader with minimal previous knowledge of CDS should be able to understand these fundamental CDS concepts. 
...
<<>>=
cds3Rates <- get.rates(date = as.Date("2014-06-24"), currency = "USD")
@
