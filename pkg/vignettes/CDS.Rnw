...
\caption{CDS cash flows for Alcoa Inc. when Spread is 60bp and and Coupon is 100bp. These have been calculated using the calculator provided by Markit}
...
\textbf{Protection leg} refers to the  If, however, there is a credit event, the cash flow would look like Figure \ref{fig:CDScreditevent}. 
