...
\caption{
This figure is the Market section cropped from Figure 2. It contains information such as Default Probability and Term Structure. Other important concepts that can be obtained from Market Section, though not directly displayed on screen, will also be introduced here, such as Hazard Rates and Survival Probability.
}
...
In ``Calculation Results,'' \textbf{Cash Settlement} is the same as \textbf{Cash Amount} in Bloomberg, \textbf{Clean Price} is same as \textbf{Price}, the \textbf{Accured Amount} in Markit and Bloomberg has different signs (one is positive while the other negative), and \textbf{Credit DV01} is same as \textbf{Spread DV01}.
