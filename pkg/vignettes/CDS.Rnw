...
\textbf{Present value} refers to the current value of a future payment. What this means is that, say if you have \$1 million today, it is worth more to you today than \$1 million 10 years from now. This is because \$1 million, if invested in, say, a risk-free asset earning 5\% a year (compounded semi-annually), will amount to \$1.63 million. So the present value of \$1.63 million ten years from now is \$1 million. The 5\% interest rate that we are using to discount the future value is known as the \textbf{discount rate}.      
