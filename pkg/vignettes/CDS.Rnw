...
So far we have only discussed pricing of CDS contracts across two periods. Let us now see how CDS are priced across multiple periods. Let's say that our CDS contract now has a tenor of five years and that each coupon payment is made quarterly. The cash flows from the protection  buyer to the seller, when there is no credit even, would look like Figure \ref{fig:CDSnoevent}
...
Moreover, we also have to account for the probability of default for each period. The cumulative probability that a company survives till the end of period \textit{n} is the probability that it survives period \textit{n}, conditional on it having survived the previous \textit{n-1} periods. The term structure of these probabilities are known as \textbf{hazard rates}.   

\subsubsection{Upfront Payment in an N-period Case}

Together, the discount factor and the survival probability are known as the \textbf{risky discount factor}. This is used in calculating the upfront value for a CDS maturing over \textit{n} periods.
