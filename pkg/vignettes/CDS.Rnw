...
\textbf{Discount factor: Using the Swap Curve or Risk-Free Curve}. The risk free rates are the discount factor used in the calculation of PV01. In calculating the price of a CDS we use the swap curve of the \textit{previous day}, and use the rates for the corresponding currency. So for CDSs denominated in USD, we will use the US treasury rates, for those in GBP we will use UK Government bond yields and so on. If we look at Figure \ref{fig:YieldCurve}, we can see the yields for different treasury bonds, at 8pm UTC, June 23 2014, which will be used for pricing the CDS of Alcoa Inc.     
...
\textbf{Survivial Probability}. The probability that the firm will default between two time periods.

\subsubsection{Present Value of the Net Expected Cash Flows}
\label{sec:PVPremiumLeg}

Now that we understand PV01 and the different components that are used to calculate it, we can understand how the present value of the protection and premium legs, and the upfront value, are calculated. The present value of the premium leg is the present value of future coupon payments. It is simply the coupon payment mutliplied by the PV01
...
So when an upfront payment is made it implies that the present value of the protection and premium legs are not equal. In the all-running contracts before the ``Big Bang Protocol'', where the coupon could be set at any level the two parties agreed to, the premium leg would naturally equal the protection leg and the present value of the contract would be zero. But when the coupon is fixed and if the fair spread is not equal to the coupon, the present values of the two legs would not be the same and an upfront payment would have to be made as compensation.    
We can then use the formula above for PV01 to calculate the principal amount (clean upfront payment) paid from the protection buyer to the seller using the following formula:
