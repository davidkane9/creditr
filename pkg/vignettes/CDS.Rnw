...
\begin{figure}[H]
\centering
\includegraphics[width=6.3in, height=2.8in]{images/AlcoaIncCDSCalculator.jpg}
\caption{
This figure is the Calculation section cropped from Figure 2, and it contains much important pricing information of the CDS.}
...
Then, let's focus on the bottom-right corner where four interesting calculations are listed. \textbf{Spread DV01} denotes the change of upfront payment if spread increases by 1. \textbf{IR CV01} denotes the change of upfront payment if the interest rate increases by 0.01\%. \textbf{Rec Risk (1\%)} denotes the change of upfront payment if the recovery risk increases by 1\%.
