...
\textbf{Spread DV01} literally stands for "Spread Dollar Value 01." It measures the change of upfront payment if spread increases by 1. Using the calculation for PV01, we can show the calculation of the main risks (exposures) of a CDS position, \textbf{Spread DV01}. Spread DV01 reflects the risk duration of a CDS trade, also known as \textbf{Sprd DV01}, \textbf{Credit DV01}, \textbf{Spread Delta}, and just \textbf{DV01}. 
...
Now we have introduced Spread DV01, IR DV01, and Rec Risk (1\%). However, before explaning what Default Exposure is, we first have to define what is a \textbf{default}.

\subsubsection{What is a Default}
...
After defining "default", we can now explain the "Default Exposure". "Default Exposure" refers to the expoure to the default of a CDS contract. It can be calculated with Recovery Rate, Notional Amount, and Principal:
...
\caption{
This figure is the Market section cropped from Figure 2. It contains much information about the interest rate curve we use to price CDS, and the term structure.
}
\label{AlcoaIncCDSMarket}
\end{figure}

To understand Market section, we first have to know why we want to take interest rates into consideration when we are pricing a CDS. Therefore, before introducing any terms on the screenshot, let's first understand \textbf{Discount Factor}.
...
After learning much about the event of default, let's go back to the screenshot and study another important element in the Market section: \textbf{Term Structure}.
...
A CDS curve is similar to the interest rate curve in that it is a curve with the fair spreads of CDS contracts for different maturities. For reference names that are liquid, these rates are available quite easily; however, if the CDS is not liquid we can use the underlying bond's price to determine the hazard rates. Moreover, if there are only a couple of maturity dates available, we can assume a flat CDS curve. We will not go into the details of how  these curves are derived but simply note that they are used in calculating the present value of the two legs and the upfront payment. From CDS curve, we can also do some other important calculations, such as hazard rates and risky discount factor.

\subsubsection{Hazard Rates and Risky Discount Factor}
...
\caption{
This is a screenshot of the Markit CDS Calculator which is available online. It calculates prices for the same CDS trading as the Bloomberg screenshot above.  We will not go into each section on the screenshot and explain every details, but we will notice some key terminology differences and distinctive features of Markit.
}
\end{figure}

\subsubsection{Terminology Differences}
First, notice that in the upper-left corner, we have an entry called "Trade Level," which is the same as \textbf{spread}. Also notice that unlikely in Bloomberg, where spread is usually presented in basis points, here in Markit we can actually select whether to present spread in basis points ("bp") or percentages. Then in section "CDS Contract Terms," "Running Cpn" just means \textbf{coupon}. Also, in section "Advanced Terms," there is an entry called "Holiday Code", where we can select "none" or "JPY" (This is because the Japanese CDS trading follows a different rules of dates, and the main difference is the trading conventions on holidays).

Second, let's move to section "Calculation Results," where several terms are different from Bloomberg's: "Cash Settlement" is same as \textbf{Cash Amount}, "Clean Price" is same as \textbf{Price}, the "Accured Amount" in Markit and Bloomberg has different signs (one is positive while the other negative), and "Credit DV01" is same as \textbf{Spread DV01}.
