...
%% Perhaps of this section is to go through almost every details of the Bloomberg and Markit screen shots, explaining all the relevant details, tying them together to the concepts we discussed above and laying the groudwork for showing off the CDS package later.

%% Right now, unfortunately, this section is a mix of that stuff (which should stay) and materials that really belongs in the N-period section above.

\subsection{Bloomberg}
\label{Bloomberg}
...
\subsection{Markit CDS Calculator}
\label{Markit}
