...
%% Introduce Bloomberg and Markit here.
...
CDS originated in the late 1980s and were popularized by a team at JPM that included Blythe Masters \citep{cdsOrigins, blythe}. Many different types of CDS have since emerged including basket default swaps (BDSs), index CDS, credit-linked notes, etc. \citep{jk}. In this \textbf{CDS} package, we focus on calculations related to single-name, or corporate, credit default swaps.
...
Suppose that a person owns a house worth \$100,000 and wants to buy property insurance on her house for one year, from January 1 to December 31. For this one-year coverage, an insurance company charges a fee of \$1,000. Call this \$1,000 the premium. In exchange for the premium payment, the insurance company agrees to pay the worth of the house---\$100,000---to the house owner if there is any property damage during that year. If damage does not occur, then the insurance company pockets the \$1,000 premium payment and doesn't have to pay anything in return. 

For our scenario, assume that the house owner pays the \$1,000 premium on January 1, the beginning of the coverage period. In the case where the property is damaged, assume also that the insurance company pays the \$100,000 compensational amount on December 31, the end of coverage period. For this example, the interest rate (discussed in Section \ref{sec:InterestRate}) is 0\%.

The cash flows for this insurance agreement depend on a probability: the probability of property damage. Since cash flows are indicative of the "winner" in the agreement---the person or company that acquires a net gain of money at the expense of the other person or company---both the house owner and the insurance company would want to know this probability, and use it to determine the \textbf{expected cash flows}.

If, for example, the house owner was extremely confident that the house would not be damaged, and thus believed the probability of damage to essentially be 0\%, then she would not want to purchase insurance. The only cash flow between the two parties would be the \$1,000 premium payment out of \emph{her} pocket for the one-year coverage. Similarly, if the insurance company thought it was extremely likely that the house would get damaged---maybe it's near a body of water that frequently floods waterfront properties---then the insurance company would probably not want to provide insurance. In that case, the insurance company has to make a \$100,000 insurance payment that greatly outweighs the premuim payment that it receives, and thus the company loses money.

Clearly, the value of the probability of damage largely affects whether or not each party agrees to this insurance agreement. This begs the question: what probability of damage would make both parties agree? In other words, what probability would make the expected cash flows for both parties are equal?
...
To determine the "fair" probability of damage at which both parties could agree on a coupon of 1\% with a notional amount of \$100,000, we set the expected cash flows of each party equal to each other:  
...
However, the above description of an insurance purchase excludes several real aspects of the insurance market. For example, it ignores the fact that insurance companies aim to \emph{profit} from the agreements they make with clients. When we equate the expected cash flows in our example, the insurance company does not expect to make a net profit, and thus the house owner does not expect to pay a net cost---which is unlike real insurance purchases. In our example, we assumed that the house owner would only purchase insurance if the cash flows were equal---
in other words, we assumed that the house owner is risk-neutral.

In reality, however, the typical person who purchases insurance is risk-adverse. In other words, she is willing to have her expected cash flow be less than that of the insurance company if only for the peace of mind that she does indeed \emph{have} insurance.

Our insurance example above also failed to explore how the insurance company would actually know that there has been, for example, a house fire. How would the company know that this \textbf{event} merits an insurance payment? The insurance company would probably want some form of proof, and the company might even have someone visit the house and assess the damage. At the beginning of the contract, both parties would need to agree how this may carry out. They would also need to agree on their definition of "property damage," and how to measure it. Clearly, this highlights the importance of a contract that specifies the terms of the agreement.

Additionally, our example only considered the following two cases: either the entire house was damaged (and the insurance company paid \$100,000), or the house was not damaged at all (and the insurance company didn't pay anything). However, there are other likely outcomes that the insurance market has to account for. For example, what if only half of the house appeared to be damaged? In that case, it would not make sense for the insurance payment to be \$100,000; instead, the insurance company should compensate only for the damaged section of the house by paying \$50,000. 
...
Instead of buying insurance against property damage, consider buying insurance against a company's inability to meet its obligations. For example, a portfolio manager from the hedge fund, Citadel, believes that Alcoa, a company, is likely to not return the money that its bondholders have lent. In other words, she is concerned that Alcoa will \textbf{default} on its bonds---not pay them back---and that Alcoa may even go bankrupt in the next year. So, she enters a one-year CDS agreement with an investment bank---say, J.P. Morgan (JPM).\footnote{For ease of reading, we desginate the buyers in our examples as female and the sellers as male.}

Keep the same timeline (January 1 - December 31) and numerical figures from our property insurance example. The interest rate is 0\%. The portfolio manager agrees to pay a \textbf{coupon} (equivalent to the premium in our property insurance example) of 1\% of the \$100,000 amount---known as the \textbf{notional amount}---for which she wants protection. Thus, the portfolio manager pays a coupon payment of \$1,000 on January 1, the beginning of the coverage period, in exchange for protection against Alcoa's default for that year. This side of the CDS contract is known as the \textbf{premium leg}, since it involves the premium payment, and the portfolio manager is known as the \textbf{protection buyer}. 

The other side of the CDS agreement is known as the \textbf{protection leg}, since it describes the protection payment, and involves JPM, the \textbf{protection seller}. If Alcoa defaults, then JPM must pay the notional amount of \$100,000 to the portfolio manager on December 31, the end of the coverage period. Note that Alcoa is known as the \textbf{reference entity} in this CDS since the protection buyer desires protection from Alcoa. The portfolio manager's specific Alcoa bonds are known collectively as the \textbf{reference obligation}.

Since the portfolio manager makes money if Alcoa deteriorates, she can be said to be \textbf{short} credit. Alternatively, since the JPM broker loses money if Alcoa collapses, he can be said to be \textbf{long} credit. In other words, the broker's returns are similar to someone who actually owns Alcoa bonds worth the notional amount.

Is this agreement a fair deal? Well, like our property insurance example, it depends on a probability: the \textbf{probability of default}. To solve for the "fair" probability of default, we need to equate the cash flows for the portfolio manager (left side of the below equation) and for JPM (right side of the below equation). In this equation, "C" stands for coupon payment, "P" is the probability of default, and "V" is the notional value.
...
This is the same equation as in the property insurance example. This shouldn't be too surprising, though, since we used the same numerical figures in this example. Since these equations are the same, we know that if the probability of default is 1\%, then this deal is fair.
...
%% Might want to think about loss given default (LGD). (1- RR) = LGD.
...
In our simple, one-period case, the spread is 100 bps on January 1 since the portfolio manager and JPM have agreed to trade with a coupon of 1\%. As the year passes and the contract matues, however, the value of the spread drops accordingly, from 100 bps on January 1 to 0 bps on December 31. If the CDS were to be traded---given to another protection buyer and protection seller in exchange for a new CDS agreement---then the new protection buyer and seller would trade at a spread of 75 bps, since the seller is only proviving coverage for 3/4 of a year, and thus the fee need only be 3/4 of the original coupon. 

This process---the drop of the spread over the duration of the contract---is known as \textbf{rolling down the curve}. 
...
So far, we have introduced a simple, one-period CDS agreement between a portfolio manager and JPM. We then introduced four factors that affect the expected cash flows for both parties: interest rate, recovery rate, upfront payment, and accrued coupon.
...
Note that for any established CDS contract, the two unknown variables are "P" and "U": the probability of default and the upfront payment. You need one to calculate the other, and vice versa.
