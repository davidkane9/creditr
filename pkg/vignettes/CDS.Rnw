...
CDS originated in the late 1980s and were popularized by a team at J.P. Morgan that included Blythe Masters \citep{cdsOrigins, blythe}. Many different types of CDS have since emerged including basket default swaps (BDSs), index CDS, credit-linked notes, etc. \citep{jk}. In this \textbf{CDS} package, we focus on calculations related to single-name, or corporate, credit default swaps.
...
In order to understand CDS, let's first talk about a simpler form of purchasing protection: property insurance. 

Suppose that person owns a house worth \$100,000 and wants to buy property insurance on the house for one year, from January 1 to December 31. For this one-year coverage, an insurance company charges a fee of \$1,000. Call this \$1,000 the premium. In exchange for the premium payment, the insurance company agrees to monetarily compensate the house owner for any property damage in the next year. For example, if a house fire occurs, then the insurance company pays \$100,000 to the house owner. If damage does not occur, then the insurance company pockets the \$1,000 premium payment and doesn't have to pay anything in return. 

For our scenario, let's assume that the house owner pays the \$1,000 premium on January 1, the beginning of the coverage period. In the case where the property is damaged, let's also assume that the insurance company pays the \$100,000 compensational amount on December 31, the end of coverage period. For this example, the interest rate (discussed in section BLANK) is 0\%.

The cash flows for this insurance agreement depend on a probability: the probability of property damage. Since cash flows are indicative of the "winner" in the agreement---the person or company that acquires a net gain of money at the expense of the other person or company---both the house owner and the insurance company would want to know this probability, and use it to determine the \textbf{expected cash flows} of the agreement. 

If, for example, the house owner was extremely confident that the house would not be damaged, and thus believed the probability of damage to essentially be 0\%, then he would not want to make this deal. The only cash flow between the two parties would be the \$1,000 premium payment out of \emph{his} pocket for the one-year coverage! Similarly, if the insurance company thought it was extremely likely that the house would get damaged---maybe it's near a body of water that frequently floods waterfront properties---then the insurance company would probably not want to make this deal. In that case, the insurance company has to make a \$100,000 compensational payment that greatly outweighs the premuim payment that it receives, and thus the company loses money.

Clearly, the value of the probability of damage largely affects whether or not each party agrees to this insurance agreement. This begs the question: what probability of damage would make both parties agree? In other words, what probability would make this agreement a fair deal where the expected cash flows for both parties are equal?

To answer this question, we need to first lay out the payment that each party expects to make. The house owner expects to pay a \$1,000 premium on January 1, regardless of whether or not damage occurs. The insurance company's compensational payment, however, is contingent upon property damage; with damage, it would be \$100,000, but without damage, it would be \$0. Thus, if "P" represents the probability of property damage, we can say the following: 
...
To determine the "fair" probability of damage at which both parties would agree to this deal, we set the expected cash flows of each party equal to each other:  
...
As we can see, the house owner expects to pay the \$1,000 premium and potentially receive the \$100,000 (based on the probability of damage), and the insurance company expects to receive \$1,000 premium and potentially pay the \$100,000 compensation payment (based on the probability of damage). Simplifying Equation 3, we get:
...
Solving for "P", we get .01, or 1\%. Therefore, if the probability of damage is equal to 1\%, then the expected cash flows for the house owner and for insurance company would be equal, and this deal would be considered fair. 

Note that in the above insurance agreement, we assumed that any property damage merited the full compensational payment of \$100,000. However, what if a house fire occured and only damaged half the house? In that case, it would not make sense for the compensational payment to be \$100,000; instead, the insurance company should compensate only for the damaged section of the house by paying \$50,000.

Given this, when the two parties initiate the insurance agreement, they need to agree on what kind of damage merits the full payment of \$100,000, and what doesn't. They would also need to agree on their definition of "property damage," and how to measure it. Clearly, this highlights the importance of a contract that specifies the terms of the agreement.

Also note that in the above insurance agreement, the house owner bought insurance for the whole house, worth a total of \$100,000. However, this need not be the case; the house owner could very well purchase insurance for half of the house, worth \$50,000. If a house fire were to occur under \emph{that} purchase, then the insurance company would have to pay a compensational amount of \$50,000---not \$100,000.

\subsection{Simple One-Period Case}
\label{sec:OnePeriod}

Instead of buying insurance against property damage, let's consider buying insurance against a company's inability to meet its obligations. For example, let's say that a portfolio manager owns \$100,000 worth of bonds from Alcoa, a company, and she is concerned that Alcoa won't return the money she lent. In other words, she is concerned that Alcoa will \textbf{default} on its bonds---not pay them back---and that Alcoa may even go bankrupt in the next year. So, she enters a one-year CDS agreement with an investment bank---say, J.P. Morgan.

What does this mean? Well, for simplicity's sake, we'll keep the same timeline (January 1 - December 31) and numerical figures from our property insurance example. We'll also stick with the assumption that interest rates are 0\%. The portfolio manager agrees to pay a \textbf{coupon} (equivalent to the premium in our property insurance example) of 1\% (or 100 basis points (bps)) of the \$100,000, known as the \textbf{notional amount} of this CDS. Thus, the portfolio manager pays \$1,000 on January 1 in exchange for protection against default of Alcoa for that year. This side of the CDS contract is known as the \textbf{premium leg}, and the portfolio manager is known as the \textbf{protection buyer}. 

The other side of the CDS agreement is known as the \textbf{protection leg} and involves J.P. Morgan, the \textbf{protection seller}. If Alcoa defaults, then J.P. Morgan must pay \$100,000 to the portfolio manager on December 31 of that year. Note that Alcoa is known as the \textbf{reference entity} in this CDS since it is the company from which the protection buyer desires protection. The portfolio manager's specific Alcoa bonds are known collectively as the \textbf{reference obligation}.
...
Is this agreement a fair deal? Well, like our property insurance example, it depends on a probability: the \textbf{probability of default}. To solve for the probability of default that would render this deal fair, we need to equate the cash flows from the portfolio manager (left side of equation) and J.P. Morgan (right side of equation). In this equation, "C" stands for coupon payment, "P" is the probability of default, and "V" is the notional amount.

\begin{equation}
 \begin{aligned}
   {\rm Insurance\hspace{3pt}buyer's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flow = Insurance\hspace{3pt}company's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flow}
    \end{aligned}
\end{equation}
\begin{equation}
 \begin{aligned}
...
The left side shows how the portfolio manager has to pay the coupon, "C", but could receive the protection leg of "V", depending on P. The right indicates the converse; J.P. Morgan will receive the coupon payment, "C", but could potentially have to pay the protection leg of "V", depending on "P". Plugging in known values ("C" = \$1,000 and "V" = \$100,000), we get:
...
This is the same equation as in the property insurance example! This shouldn't be too surprising, though, since we used the same numerical figures in this example. Since these equations are the same, we know that if the probability of default is 1\%, then this deal is considered fair.

\subsubsection{Interest Rate}
\label{sec:InterestRate}

%% Not sure what sort of interest rate definition we should provide at this point. Singular or plural?

Up until now, we have assumed that the interest rate is 0\% for our CDS example. The \textbf{interest rate} is a benchmark rate that represents the return that investors can expect without much risk. How might the cash flows in the CDS agreement change if the interest rate was \emph{not} 0\%---instead, say 2\%?

First, let's review our CDS agreement: the portfolio manager pays the premium leg on January 1 and J.P. Morgan pays the protection leg (if Alcoa defaults) on December 31. J.P. Morgan receives the \$1,000 coupon a full \emph{year} before the portfolio manager would receive the \$100,000 payment if Alcoa defaults. In an environment where the interest rate is 2\%, J.P. Morgan could theoretically invest the \$1,000 sum starting January 1 and earn interest for that year:
...
By December 31, the \$1,000 could grow to \$1,020, which gives J.P. Morgan an extra \$20 that the portfolio manager won't receive because she doesn't get a payment---if at all---until the end of the year. As such, the \$1,000 coupon has more value \emph{per} \emph{dollar} than the \$100,000 payment. If the interest rate is not 0\%, then we need to adjust the cash flows accordingly to account for the extra money that J.P. Morgan gains. Otherwise, the portfolio manager's expected cash flow is larger that that of J.P. Morgan---and why would the portfolio manager want to enter this agreement if it were unfair on her own end?
...
Thus, the new coupon value that would account for a world in which the interest rate is 2\% is \$980.39. If the portfolio manager gives J.P. Morgan \$980.39 on January 1, J.P. Morgan can invest that sum and grow it to \$1,000 by December 31, when J.P. Morgan would pay the protection leg if necessary. 

For a generic case, here are the cash flows when the interest rate is not 0\%. "P", again, refers to the probability of default; "V" represents the notional amount; "i" stands for the interest rate; and "C" is the coupon payment (before discounting).
...
Now that the coupon rate has changed to adjust for an environment with 2\% interest rate, the probability of default that both parties would want to agree on to make this deal changes, as well. As we can see, P = .98\% (as opposed to 1\% in an environment with 0\% interest rate).

\subsubsection{Recovery Rate}
\label{sec:RecoveryRate}

Recall that in our property insurance example, we mentioned that some cases---such as a house fire that burns half of a house---may not merit a full compensation payment equal to the notional amount (in that case, \$100,000). Similarly, when a company defaults, it can usually retain some of its assets and distribute that money among those to whom the company owes money. In other words, companies that default are usually able to partially compensate its bondholders. In such a scenario, a protection seller for that company would only have to pay the fraction of the notional amount that the reference entity, or company did \emph{not} pay, in the case of default.

Let's exemplify using our simple, one-period CDS: suppose that Alcoa defaults during the one-year CDS contract, but it's able to give the portfolio manager \$50,000 of the \$100,000 it owes her. In that case, J.P. Morgan would only have to pay the portfolio manager another \$50,000 (not the notional amount of \$100,000) to compensate for her monetary loss from Alcoa's default. The ratio of the amount a bondholder receives after a company's default compared to the amount the bondholder \emph{should} have received (with no default) is known as the \textbf{recovery rate}. So, in the example described above, the recovery rate is 50\%.

Let's assume that, once again, the interest rate is 0\%, and thus not a factor in the expected cash flows for the portfolio manager and J.P. Morgan. A fair deal is one in which these two values are equal. In a CDS in which the company is expected to have a recovery rate, we could lay out the cash flows as in the following equation. Like before, the left side represents the cash flows for the portfolio manager and the right side represents the cash flows for J.P. Morgan. (Note that "r" refers to the recovery rate and, as before, "P" is the probability of default, "C" is the coupon payment, and "V" is the notional amount.) 

\begin{equation}
 \begin{aligned}
   {\rm - C + (P)([1-r]*V) = C - (P)([1-r]*V)}
    \end{aligned}
\end{equation}

Although this equation may look more complicated than other equations we've introduced, note that this equation is very similar to Equation 6. In fact, the only difference (multiplying "V" by (1-r)) accounts for the fact that the protection leg is the \emph{fraction} of the notional amount that the protection seller has to pay (since there is a non-zero recovery rate). Since recovery rate specifies the fraction of the notional amount that the \emph{ reference entity} is able to pay, we need to multiply "V" by (1-r), not just "r", to find the true value of the protection leg.

To solve for the probability of default that would make this deal fair, let's plug in the known values in the case where the recovery rate is 50\%:

\begin{equation}
 \begin{aligned}
   {\rm - \$1,000 + (P)([1 - 0.5]*\$100,000) = \$1,000 - (P)([1 - 0.5]*\$100,000)}
    \end{aligned}
\end{equation}
...
In such a scenario, the default probability that both parties would need to agree to for this to be fair is 2\%.

\subsubsection{Upfront payment}
\label{sec:Upfront}

Let's go back to our simple CDS example---0\% interest rate, 0\% recovery rate. So far, we have looked at different scenarios and calculated an ideal probability of default which would make both parties happy. Or, at least, the CDS agreement would be fair. In the simple one-period CDS example, this ideal probability was calculated to be 1\%.

However, what if the \emph{actual} probability of default is 2\%? 0.5\%? Clearly, this is no longer a fair deal. How do we construct a fair deal with equal expected cash flows from the portfolio manager and from J.P. Morgan?

Let's consider the case in which the actual probability of defaut is 0.5\%. The expected payment of the portfolio manager at the end of the year will be greater than J.P. Morgans's expected payment since the probability of default (and thus the probility that J.P. Morgan will have to pay out any money at all) is less than the probability that makes the deal fair: 1\%. A simple solution to this issue would be to lower the portfolio manager's coupon. 
...
\begin{equation}
 \begin{aligned}
   {\rm -\hspace{3pt}\$500 + ((P = 0.5\%) * {\$100,000})  = \$500 - ((P = .5\%) * {\$100,000})}
    \end{aligned}
\end{equation}

Since the probability of default and the coupon are directly related, we divide the coupon in half (from \$1,000 to \$500) when and if the probability of default is divided in half (from 1\% to 0.5\%). This is surely a viable solution.

However, let's say that, by CDS market conventions, all CDS are traded at a standard coupon of 1\%. If the \emph{actual} probability of Alcoa's default is less 1\% (0.5\% in this case), how do we modify the CDS agreement to once again make it a fair trade for both parties?

The answer lies in an \textbf{upfront payment}---a payment made from one party to the other at the beginning of a contract (in this case, January 1). If the probability of Alcoa's default is 0.5\%, then J.P. Morgan has to compensate the protection buyer for paying a coupon of 1\% instead of a coupon of 0.5\%. Since the difference between the protection buyer's fair payment (\$500 at 0.5\%) and the payment she has to make due to convention (\$1,000 at 1\%) is \$500, J.P. Morgan's upfront payment to the protection buyer should be \$500. Another way of way of thinking about the upfront payment is to consider the difference in expected cash flows between the two parties. Since J.P. Morgan's expected cash flow is larger than that of the portfolio manager, the upfront payment ("X") is positive on the left side of the equation (the portfolio manager's expected cash flow) and negative on the other side (J.P. Morgan's expected cash flow). As before, "C" designates the coupon payment, "P" is the probability of default, and "V" is the notional amount.
...
\begin{equation}
 \begin{aligned}
   {\rm X - \$1,000 + (.005 * \$100,000) = \$1,000 - (.005 * \$100,000) - X}
    \end{aligned}
\end{equation}
...
\begin{equation}
 \begin{aligned}
   {\rm (2)X + (2)(\$500) = (2)*\$1,000}
    \end{aligned}
\end{equation}

As such, \$500 is the correct upfront payment for this situation.

That way, with an upfront payment, the two parties can trade according to CDS convention and still create a fair deal in which the expected payments from each party are equal. Let's assume that both the upfront payment and the coupon payment are made at the same time: on January 1, the first day of the one-year contract. In this case, since the portfolio manager would have to pay J.P. Morgan \$1,000, and J.P. Morgan would have to pay the buyer \$500, the two parties can just agree to have one equivalent cash flow: the protection buyer paying \$500 to J.P. Morgan.
...
%% Need to explain dirty upfront and clean upfront.

\subsubsection{Accrued Coupon}
\label{sec:AccruedCoupon}

Again, back to our simple, one-period CDS example. We have so far assumed that our CDS agreement between the portfolio manager and J.P. Morgan begins on January 1 and ends on December 31 of the same year. However, what if this agreement wasn't made exactly on January 1? What if it was made halfway through the year, on June 30? Let's assume the probability of default for those six months is indeed 1\%---the probability that renders the CDS contract fair. Under our current description of a CDS agreement, this implies that the portfolio manager would have to pay the same coupon of 1\% (\$1,000) on January 1 for only receiving six months (instead of one year) of protection coverage. Now, the expected payment of the portfolio manager is greater than that of J.P. Morgan.

To account for this, let's first calculate the amount of the coupon that the portfolio manager is unfairly paying---that is, the fraction of the coupon payment that accounts for the first six months of the year (January 1 - June 30), which she does not receive coverage for. This fraction of the coupon payment is known as the \textbf{accrued coupon}.

\begin{equation}
 \begin{aligned}
  {\rm Accrued = \frac{180}{360}*\frac{1.0}{100}*\$100,000 = \$500}
    \end{aligned}
\end{equation}

Note that we just divided the coupon payment in half, which makes sense since January 1 - June 30 consists of half of the year. Note also that we are dividing 180 by 360 instead of 365 in the above calculation. This particular CDS contract can be said to have a 30/365 \textbf{day count convention}, meaning that the accrued coupon calculations are based on the assumptions that there are 30 days in a month and 360 days in a year. This simplication makes calculations much easier. 

%% Why, other than for simplicity's sake, do people use the 30/365 day count convention?

The portfolio manager does not want (and understandably does not have the obligation) to pay for the six months she did not receive coverage for. Instead, it is \emph{J.P. Morgan} who pays the accrued coupon to the portfolio manager; after this adjustment, the expected payments from each party are once again equal. Since the portfolio manager is already paying J.P. Morgan a coupon of \$1,000 and now, in this case where the CDS contract initiates on June 30, J.P. Morgan owes the portfolio manager \$500 of accrued interest, the two parties can agree to one equivalent cash flow: \$500 from the portfolio manager to J.P. Morgan.
...
So far, we have introduced a simple, one-period CDS agreement between a portfolio manager and J.P. Morgan. We then introduced four factors that affect the expected cash flows for both parties: interest rate, recovery rate, upfront payment, and accrued coupon.

In a real-world scenario, we'd have to look at \emph{all} these factors at the same time, and come up with a way to show cash flows that take all four factors into account.

Combining Equations 1, 2, and 3, we get a master equation that accounts for interest rate ("i"), recovery rate ("r"), and upfront payment ("X"). Accrued coupon can be calculated separately based on the date the CDS was initiated. (As before, "C" stands for the coupon payment, "P" for the probability of default, and "V" for the notional amount.)

\begin{equation}
 \begin{aligned}
   {\rm X -\hspace{3pt}\frac{C}{(1 + i)} + (P)([1-r] * V)  = \frac{C}{(1 + i)} - (P)([1-r] * V) - X}
    \end{aligned}
\end{equation}

Note that for any established CDS contract, the two unknown variables are "P" and "X": the probability of default and the upfront payment. You need one to calculate the other, and vice versa.

By looking at this equation, we can see that in order for any CDS trades to be made, there needs be a general consensus on the value of the recovery rate for a certain reference entity and the value of the interest rate at a given time. In the CDS market, these values are determined and set by an regulatory organization known as the International Swaps and Derivatives Association (ISDA). 
...
%% ACCRUED INTEREST + DISCOUNTED RATES + UPFRONT PAYMENTS: Thus, if the CDS contract was initiated on June 30, then the total upfront payment made by J.P. Morgan to the portfolio manager would be \$495 (which is equal to \$500/(1+ 1\%) rounded to nearest dollar) + \$250 = \$745. This value is known as the \textbf{dirty upfront}, since it specifies the actual upfront payment that (in this case) the protection seller will pay the protection buyer. The \textbf{clean upfront}, on the other hand, does not take accrued interest into account, and (in this case) is equal to \$495. Note that the portion of the upfront payment that accounts for a lower probability of default than 1\% is discounted by the discount rate, and the portion that accounts for the accured interest is not. This is because the accrued interest should be worth \$250 \emph{now} (June 30), and the other portion of the upfront payment should be worth \$500 on December 31.

\subsection{Two-Period Case}
\label{sec:TwoPeriod}

Up until now, we have described various aspects of a CDS agreement that has one coupon payment on January 1. However, most CDS include more than one coupon payment. Let's explore an agreement which is the same as the simple, one-period CDS example in the previous section with the exception of the duration: two years. This CDS has two coupon payments: one on January 1 of the first year and one on January 1 of the second year. For this case, let's assume that the interest rate is 0\% and the probability of default is constant for both years of the contract. If the notional amount is still \$100,000 and the standardized coupon is still 1\% (so the buyer will make two \$1,000 coupon payments), then the probability of default would have to be 2\% for this to be a fair deal (plugging in values for Equation BLANK):
...
%% In this example, however, let's say that the probability default is actually 5\%, instead of 4\%, and the standardized coupon must remain at \%1. What would be an appropriate upfront payment for this type of contract?

%% In this case, it is the protection buyer that has to pay the upfront payment. The amount of money that J.P. Morgan expects to pay increases as the probability of default increases, whereas the total payment for two coupon payments of \$2,000 is set at \$4,000 and does not change. Thus, the portfolio manager will have to pay the difference between the total coupon payment that would merit a "fair deal" with a 4\% default probability and the standardized total coupon payment; or, the portfolio manager has to pay the difference in expected payment values(see Equation BLANK).

%% \begin{equation}
 %% \begin{aligned}
  %% {\rm Upfront\hspace{3pt}Payment = {|(\$4,000) - ((d = .05) * \$100,000) = \$1,000|}}
    %% \end{aligned}
%% \end{equation}

%% The upfront payment would then be \$1,000. Note, again, that this is assuming an interest rate of 0\%, so discounting is not necessary.
...
If the probability of default changes from year one to year two, then this can make our calculation of the upfront payment a bit more complex. If, for example, the probability of default during year one is 2\%, and the probability of default during year two is 4\%, then the expected payment from J.P. Morgan to the portfolio manager would be the expected payments for year one averaged with the expected payment for year two \emph{assuming} \emph{Alcoa} \emph{doesn't} \emph{default} \emph{during} \emph{year} \emph{one}. See calculation below:
...
Given that \$4,400 is the expected payment for J.P. Morgan, that of our portfolio manager should be equivalent. Since the coupon payment is set 1\%, and since there are two coupon payments, the upfront payment (made by the portfolio manager) can be determiend by finding the difference:
...
Let's add another layer of complexity. What if we have the same exact scenario as described above, but the interest rate is not 0\%? What if the interest rate \emph{changes} from year one to year two?

First let's assume that the interest rate remains constant over the two years at 2\%, compounded annually. So, if J.P. Morgan were to invest its upfront payment in a treasury bond earning the interest rate, their \$2,321 would change (rounded to the nearest dollar):
...
%% \$2,450/$(1 + .02)^2$ = 2354 86

%% Explain how this changes the calculation.

%% Explain if interest rates are changing.
...
To understand the different variables used to determine cash flows and bond pricing, let's look at actual data for an Alcoa bond that will mature, or be paid out, five years from now (since the data descibes a bond that was initiated seven years ago). 
