...
\centering
\includegraphics[width=4.7in, height=3.2in]{images/CDSnotional2.png}
...
Although the cash flows involved in a CDS seem pretty simple in figure \ref{fig:cashFlow}, the underlying mechanics of a CDS are a bit more complex. To understand this, let us look at the actual CDS data of Alcoa Inc. from Bloomberg\footnote{The two most commonly used sources for CDS related data are Bloomberg and Markit, which we will discuss extensively in a later section.}. Figure \ref{fig:CDSalcoa} below displays many different variables such as RED code, Reference Entity et al. on the CDS of Alcoa Inc. for June 02, 2014.   
...
Some of these variables seem rather straighforward. \textbf{Reference Entity} refers to the name of the company from which the protection buyer in the CDS contract has purchased a bond. In our example, the Reference Entity is Alcoa Inc. The \textbf{RED (Reference Entity Database) Code} is an alphanumeric code that refers to a specific company. Each entity/seniority pair has a unique six-digit RED
code. Each deliverable bond has a nine-digit RED code, where the first six digits match the six-digit code of the associated entity/seniority. Of the variables in this figure, we must pay attention to certain key terms:

%% Need to work on paragraph above.
