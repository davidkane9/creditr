...
To understand the different variables used to determine cash flows and bond pricing, let's look at actual data for a bond of Alcoa Inc., that has a maturity date five years from now. This data has been retrieved from Bloomberg, one of the most widely used sources of financial data.
...
If we look at the right of Figure \ref{fig:AlcoaBond}, under `Bond Ratings', we can certain symbols provided by organizations such as Moody's, S\&P and Fitch. These symbols are known as \textbf{credit ratings} and are indicators of a company's \textbf{credit risk}. When a company like Alcoa Inc. issues bonds, there is always the possibility that it may not be able to meet its debt obligations. The risk that a company will be unable to meet its debt obligations is known as the credit risk. The creditworthiness of a company is often represented by the \textbf{credit rating}---a score determined by rating agencies such as Standard \& Poor's (S\&P), Moody's and Fitch. We will not go into the details of how these ratings are determined; we only note that an Investment Grade bond has a rating of BBB- or higher from S\&P or Baa3 or higher from Moody's.
...
A risk-averse investor, like our pension fund portfolio manager, would naturally want to purchase senior bonds from a company that has a low credit risk---a company like Alcoa Inc., which has a rating of BBB- from S\&P and Ba1 from Moody's. A company with a low credit risk is considered to be \textbf{credit worthy} and its bonds are classified as \textbf{Investment Grade} (IG). Bonds that have a rating below this are termed as \textbf{speculative grade} bonds or \textbf{junk} bonds.  

Another important term associated with credit risk is a \textbf{credit event}. A credit event can signify the risk of a company's bankruptcy and resulting default, but the term can also expand to include failure to pay out debt within a certain amount of time, a credit downgrade or the confiscation of assets, among other events. In a later section, we will see the exact events that constitute a credit event.

\subsubsection{Risk-free Bonds}
\label{sec:RiskFreeBonds}

Bonds issues by the US Government (known as \text{Treasuries}) are generally considered to be \textbf{risk-free}, as the probability of the US government not meeting its debt obligations is almost negligible. Even bonds issued by Japan (Japanese Government Bonds or JGBs) and Britain (Gilts) are considered to be risk-free. Therefore, the interest paid by sovereign bonds is an important benchmark for pricing of corporate bonds or even the bonds of other governments.

Bonds of Alcoa Inc., or any corporation for that matter, are generally riskier than US treasury bonds. A rational investor would want a riskier bond to pay a higher interest rate than a US treasury, or else it wouldn't make sense for her to take on extra risk when she could get the same interest payment at no risk at all. She wants the expected return of a corporate bond to be the same as the expected return of a risk-free bond. Therefore, the minimum amount the risky bond would have to pay over the risk-free asset, for the expected return to be equivalent, is known as the \textbf{risk premium}. So the interest premium that an investor would want from a bond would change as the risk-free rate changes. It is important to understand the intuition behind this before we explain how bonds are priced. 
...
`M' is the principal payment and `C' refers to the coupon payment of \$25000. `i' is the discount rate and is also called the \textbf{yield to maturity(YTM)} or just \textbf{yield}. It is the required interest or discount rate for the present value of a future payment to be equal to the bond price.
