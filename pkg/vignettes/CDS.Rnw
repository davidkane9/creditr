...
Moreover, we also have to account for the probability of default for each period. Together, these are known as the \textbf{risky discount factor}. 




Therefore, the cash flow, when there is no credit even, would look like Figure \ref{fig:CDSnoevent}
