...
Figure 2 below displays data on the CDS of Alcoa Inc on June 02, 2014 from Bloomberg. The two most commonly used sources for CDS related data are Bloomberg and Markit. In the screenshot below, we can see many of the key variables that are used in a CDS contract between two counterparties. In this section we will describe these variables and how they matter to our portfolio manager when she evaluates a CDS. :\footnote{Some of the
