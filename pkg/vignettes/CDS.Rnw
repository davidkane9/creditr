...
Suppose that a person owns a \$100,000 house and wants to buy property insurance on her house from January 1 to December 31 of the same year. For one year of coverage, an insurance company charges a fee of \$1,000. Call this \$1,000 the premium. In exchange for the premium, the insurance company agrees to pay the worth of the house---\$100,000---to the house owner if there is any property damage during that year. If damage does not occur, then the insurance company pockets the \$1,000 premium and pays nothing in return.

In this insurance agreement, the house owner pays the premium on January 1, the beginning of the coverage period. If the property is damaged, the insurance company pays the \$100,000 on December 31, the end of coverage period. The current interest rate (discussed in Section \ref{sec:InterestRate}) is 0\%.

The cash flows for the exchange depend on a probability: the probability of property damage. Consider the three possible cash flow outcomes: the house owner could receive more cash than she pays, the insurance company could receive more cash than it pays, or the two parties could pay the same cash amount. Since cash flows can determine if there is a "winner" in the agreement---a person or company who acquires a net gain of money at the expense of the other person or company---both the house owner and the insurance company would want to know the probability of property damage and use it to calculate their \textbf{expected cash flows}. Based on their calculations, the house owner will decide if she wants to buy the insurance, and the insurance company will decide if it wants to sell the insurance. 
...
The house owner would only want to buy insurance if she receives more cash than she pays or if the expected cash flows are equal. The insurance company would only want to sell insurance if it receives more cash than it pays or if the expected cash flows are equal. Clearly, each party would \emph{prefer} to make a net gain of money, but the opposite party would probably not agree to such a deal in which it expects to lose money. As such, assume that in order to have \emph{both} parties agree to the agreement, the expected cash flows for each party have to be equal. 

Given the assumptions above, what can we conclude regarding what each party thinks about the probability of default if the deal is made? If the house owner agrees to pay a \$1,000 premium for one year of coverage, and the insurance company agrees to pay \$100,000 if property damage occurs during that year, then we know that both parties agreed on a probability of damage that rendered the expected cash flows equal (since the probability of damage determines the cash flows). It would be useful to know this "fair" probability of damage---the probability that equates both cash flows, thus making the insurance agreement fair. How might we determine this value? 

First, review our cash flows. The house owner expects to pay a \$1,000 premium on January 1, regardless of whether or not damage occurs. The insurance company's payment, however, is contingent upon property damage; with damage, the company would pay \$100,000 on December 31, but without damage, it would pay nothing. We assumed that the house owner and insurance company could only agree to the same insurance agreement if the expected cash flows were equal; therefore, in order to determine the "fair" probability of damage at which both parties agreed, we set the cash flows equal to each other. ("P" represents the probability of property damage.)
...
The house owner expects to pay the \$1,000 premium and potentially receive a \$100,000 payment (depending on damage, which we account for using "P", the damage probability), and the insurance company expects to receive the \$1,000 premium and potentially pay \$100,000 (depending on damage). Simplifying Equation \ref{eqn:PropertyInsurance}:
...
Solving for "P", we get .01, or 1\%. Therefore, by agreeing to such an agreement, both parties implied that they believe the probability of property damage is 1\%.

\subsubsection{Property Insurance Complications}
\label{sec:PIComplications}

Unfortunately, the above insurance purchase---although relatively simple---excludes several real aspects of the insurance market. For example, insurance companies aim to \emph{profit} from selling insurance; if they charge as much as they expect to pay, then they can't expect to make any money as an insurance company. In our example, we ignored this reality when we assumed that the two parties would only strike a deal if the expected cash flows were equal.

Unlike clients in the insurance market, we assumed that the person purchasing insurance---the house owner---was \textbf{risk-neutral}. In other words, we said that she would only buy insurance if she expected to receive as much money as the insurance company. In reality, however, the typical person who purchases insurance is \textbf{risk-adverse}. She is generally willing to expect to receive less money than she pays if only for the peace of mind that she has insurance, should something unlikely happen.
...
Instead of buying insurance against property damage, consider buying insurance against a company's inability to meet its obligations. For example, a portfolio manager from the hedge fund, Citadel, believes that the company, Alcoa, isn't likely to return the money that its bondholders have lent. In other words, she believes that Alcoa will \textbf{default} on its bonds---not pay them back---and she feels confident enough that she wants to bet pn it. So, she enters a one-year CDS agreement with an investment bank---say, J.P. Morgan (JPM)\footnote{For ease of reading, we desginate the buyers in our examples as female and the sellers in our examples as male.}---in which she purchases protection on \$100,000, an amount known as the \textbf{notional amount} of the CDS. Note that bondholders can purchase protection on their bonds (i.e. an Alcoa bondholder can enter a CDS in which she purchases protection against Alcoa's default), but this does not have to be the case. In our example, the portfolio manager does not own Alcoa bonds. 

Keep the same timeline (January 1 to December 31) and numerical figures from our property insurance example. The interest rate is still 0\%. In exchange for protection on the \$100,000, the portfolio manager agrees to pay a \textbf{coupon} (equivalent to the premium in our property insurance example) of 1\% of the notional amount. The coupon payment of \$1,000 is paid on January 1, the beginning of the coverage period. This side of the CDS is called the \textbf{premium leg}, and the portfolio manager is known as the \textbf{protection buyer}, since she is purchasing purchasing protection.

The other side of the CDS agreement is known as the \textbf{protection leg}, since it describes the protection in case of default, and involves the JPM broker, called the \textbf{protection seller}. If Alcoa defaults, the JPM broker pays the notional amount of \$100,000 to the portfolio manager on December 31, the end of the coverage period. Alcoa is known as the \textbf{reference entity} in this CDS since the protection buyer desires protection from Alcoa. 

Note that we are using the terms "premium" and "coupon" synonymously. Premium is commonly used to describe the periodic fee paid by the protection buyer, which is why this side of a CDS is called the premium leg. However, even though coupon is commonly used to describe the periodic fee paid by a person who owns bonds, coupon can also be used to describe the periodic fee in a CDS context. For clarity, we will just use coupon in this paper when referring to the periodic fee paid by the protection buyer.

Since the portfolio manager makes money if Alcoa deteriorates, she can be said to be \textbf{short} credit. Shorting is generally a method of profiting from another entity's failure. Alternatively, since the JPM broker loses money if Alcoa collapses, he can be said to be \textbf{long} credit. In other words, the broker's returns are similar to those of a person who owns Alcoa bonds. If Alcoa does well, the JPM broker does well, and if Alcoa doesn't, then the broker doesn't either.

Clearly, neither party wants to expect to lose money. So, the portfolio manager would only want to buy protection if she receives more cash than she pays or just as much as she pays. The JPM broker would only want to sell protection if he receives more cash than he pays or just as much as he pays. Thus, the only way both parties agree is if they both expect to receive just as much money as they each pay---in other words, if their expected cash flows are equal.

Assume that both parties agree to the above CDS. As such, we can conclude that both parties agreed to act as if they believe the \textbf{probability of default} to be one that equates the expected cash flows. To determine the probability of default that made the expected CDS cash flows equal, we set them equal to each other and solve for "P", now used for the probability of default. ("C" stands for coupon payment and "V" is the notional value.)

\begin{equation}
 \begin{aligned}
   {\rm Portfolio\hspace{3pt}manager's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flow = JPM\hspace{3pt}broker's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flow}
    \end{aligned}
\end{equation}
\begin{equation}
 \begin{aligned}
...
This is the same equation as in the property insurance example. This shouldn't be too surprising, though, since we used the same numerical figures in this example. Since the probability of damage that we solved for in the property insurance example was 1\%, the probability of default in the equation above for this simple one-period CDS example is also 1\%. Note that this probability of default is not necessarily the true default probability; instead, it is the default probability that both parties implied by agreeing to this particular CDS. Since neither party wants to expect to lose money, the portfolio manager wanted to make a deal implying a probability of default of 1\% or higher, whereas the JPM broker wanted to make a deal implying a probability of default of 1\% or lower. Clearly, the overlap is 1\%.

\subsubsection{Simple One-Period Case Complications}
\label{OnePeriodComp}

Similar to the property insurance example, we made several assumptions in our simple one-period CDS case that are not consistent with the CDS market. By saying that a guarenteed coupon of \$1,000 and a 1\% chance of receiving \$100,000 are essentially equal, we are assuming that both parties are risk-neutral. In reality, however, most protection sellers and buyers would not think that a guarenteed \$1,000 and a 1\% chance of \$100,000 are exactly equal and may account for that risk accordingly.

We also assumed that the protection leg would only be paid out in the case of default and bankruptcy. However, depending on the particular CDS, the protection leg could be paid out even if the reference entity only defaults and doesn't go bankrupt. In fact, there are several scenarios that can be considered a \textbf{credit events}---occurences that merits the payout of the protection leg in a CDS. Besides default and bankruptcy, common credit events include failure to return money to bondholders within a certain amount of time, a credit rating downgrade (explained in Section \ref{sec:RiskBonds} and the confiscation of assets, among others.

For the rest of the paper, we will refer to the above example as the simple one-period CDS. The following four sections explore four separate complications in the simple one-period case---interest rate, recovery rate, accrued coupon and upfront payment---that each merit a separate section and example. 

\subsubsection{Interest Rate}
\label{sec:InterestRate}

Up until now, we have assumed that the interest rate is 0\% in the simple one-period CDS. The \textbf{interest rate} is a benchmark rate that participants in the CDS market use to discount cash flows. How might the cash flows in the CDS agreement change if the interest rate was \emph{not} 0\%---instead, say 2\%?

First, look at our CDS agreement: the portfolio manager pays the premium leg on January 1 and the JPM broker pays the protection leg (if Alcoa defaults) on December 31. The broker receives the \$1,000 coupon a full \emph{year} before the portfolio manager would receive the \$100,000 payment if Alcoa defaults. In an environment where the interest rate is 2\%, the broker could theoretically invest the \$1,000 sum starting January 1 and earn interest for that year:
...
By December 31, the \$1,000 would grow to \$1,020, which gives the broker an extra \$20. If the interest rate is not 0\%, then we need to adjust the cash flows accordingly to account for the extra money that the JPM broker would gain. Otherwise, the broker's expected cash flow is larger that of the portfolio manager---and why would the portfolio manager want to enter this agreement if it were unfair on her own end?
...
Thus, the new coupon value that would account for a world in which the interest rate is 2\% is \$980.39. If the portfolio manager gives \$980.39 to the JPM broker on January 1, JPM can invest that sum and grow it to \$1,000 by December 31, when the broker would pay the protection leg if necessary. 
...
Now that the coupon rate has changed to adjust for an environment with 2\% interest rate, the probability of default that would make both parties agree to a coupon of 1\% for a notional amount of \$100,000 changes, as well. As we can see, P = .98\% (as opposed to 1\% in an environment with 0\% interest rate). In other words, if this CDS is initiated, then that signifies that both parties are willing to act as if they believe probability of default to be .98\%.

\subsubsection{Recovery Rate}
\label{sec:RecoveryRate}

Recall that in our property insurance example, we mentioned that some cases---such as a house fire that burns half of a house---may not merit a full compensation payment equal to the notional amount (in that case, \$100,000). Similarly, when a company defaults, an \textbf{auction} occurs in which the company's bonds are sold at a lower, but usually substantial, price. In such a scenario, the protection leg in a CDS consists only of the 
\textbf{loss given default} (LGD), or the notional amount's decrease in value (based on auction prices). 

We'll begin with our simple one-period CDS with an interest rate of 0\%. Suppose that Alcoa defaults during the one-year CDS contract, and now that any Alcoa bond is worth less, a \$100 bond can now be sold at the auction for a price of \$55. In other words, the recovery rate for Alcoa is 55\%. In that case, the JPM broker would only have to pay the portfolio manager \$45,000 (not the notional amount of \$100,000) because bondholders who have purchased protection are able to retain 55\% of their bonds' worth. 
...
So, we need to factor the recovery rate into our calculation of cash flows because it changes the value of the protection leg in the simple one-period case. Since the portfolio manager and the JPM broker will only both agree to this CDS if the expected cash flows are equal, we set the portfolio manager's cash flows (left side of \ref{eqn:RecoveryRate} below) and the JPM broker's cash flows (right side of \ref{eqn:RecoveryRate} below) equal to each other. ("RR" refers to the recovery rate and, as before, "P" is the probability of default, "C" is the coupon payment, and "V" is the notional value.)
...
In such a scenario, the fact that a deal was made indicates that both parties believe that the default probability is 1.8\% (rounded to the nearest tenth)---or least they're willing to make a deal that assumes a default probability of that value.

\subsubsection{Accrued Coupon}
\label{sec:AccruedCoupon}

Back to our simple one-period CDS example---no interest rate, no recovery rate. For this scenario, both the premium leg and the protection leg are paid on December 31. We have so far assumed that our CDS agreement between the portfolio manager and the JPM broker begins on January 1 and ends on December 31 of the same year. However, what if this agreement wasn't made exactly on January 1? What if it was made on March 31, one quarter through the year? Using our simple one-period CDS example, this implies that the portfolio manager would have to pay the same coupon of 1\% (\$1,000) on December 31 for only receiving nine months (instead of one year) of protection coverage. Now, the portfolio manager expects to pay more than the JPM broker, so the portfolio manager would not agree to buy protection.

To once again make this a deal where expected cash flows are equal---and therefore make this a deal that both parties would agree too---we first calculate the amount of the coupon that the portfolio manager is unfairly paying. In other words, we need to determine the fraction of the coupon payment that accounts for the first three months of the year (January 1 to March 31), which she does not receive coverage for. This fraction of the coupon payment is known as the \textbf{accrued coupon}.
...
Note that we are dividing 90 by 360 instead of 365 in the above calculation. This particular CDS contract can be said to have a 30/365 \textbf{day count convention}, which means that the accrued coupon calculations are based on the assumptions that there are 30 days in a month and 360 days in a year. For example, according to this convention, if a CDS agreement was initiated on February 28 (during a non-leap year), then we would multiply that coupon payment by 60/360 and thereby imply that there are 60 days in between January 1 and February 28 (when, in reality, there are only 59). If a CDS agreement was initiated on October 31, and it was a leap year, we would multiply the coupon payment by 300/360 and thereby imply that there are 300 days in between January 1 and October 31 (even though there are actually 304). As such, this day count convention takes in the number of months that have passed (two in the first example, ten in the second) and multiplies that number by 30 to get the number of days. This day count convention makes calculations much easier.

The portfolio manager does not want (and understandably does not have the obligation) to pay for the three months she did not receive coverage for. Instead, it is the \emph{JPM} \emph{broker} who pays the accrued coupon to the portfolio manager; after this adjustment, the expected cash flows from each party are once again equal. So, the portfolio manager pays a coupon payment of \$1,000 to the JPM broker and the broker now pays \$250 of accrued coupon to the portfolio manager.

%% INCLUDE ACCRUED COUPON IN NEXT SECTION!!!!!!
...
Back to our simple one-period CDS example beginning on January 1 and ending on December 31, with no interest rate and no recovery rate. So far, we have considered scenarios in which both parties conveniently believe that the probabilit of default is 1\%---which also happens to be the probability that equates the expected cash flows and thus makes the two parties agree on the CDS. Clearly, this is quite an assumption. However, what if both parties believe that the probability of default is 2\%? Or 0.8\%? Clearly, with either of those probabilities, the expected cash flows would no longer be equal because the JPM broker would either expect to pay more or less than the portfolio manager. How do we construct a deal (based off our simple one-period CDS) with equal expected cash flows that have, say, a default probability of 0.8\%?

A simple solution to would be to lower the portfolio manager's coupon payment from \$1,000 to \$800. 
...
As we can see, the expected cash flows are equal when the coupon is adjusted to \$800.

However, consider an environment in which CDS must be traded at a coupon of 1\%. (This resembles real CDS market conventions---discussed later in this section---which is why we are going to the trouble of replicating it.) If both parties believe the probability of default to be 0.8\% (or, at least, they want to make a trade that assumed that probability), how do we modify our simple one-period CDS agreement (without modifying the coupon) to once again equate expected cash flows?

The answer lies in an \textbf{upfront payment}---a payment made from one party to the other at the beginning of a contract (in this case, January 1). If the probability of Alcoa's default is 0.8\%, then the JPM broker has to compensate the protection buyer for paying a coupon of 1\% instead of a coupon of 0.8\%. Since the difference between the protection buyer's desired coupon payment (\$800 at 0.8\%) and the payment she has to make due to convention (\$1,000 at 1\%) is \$200, the JPM broker's upfront payment to the protection buyer should be \$200. Another way of way of thinking about the upfront payment is to consider the difference in expected cash flows between the two parties. Since the broker's expected cash flow is larger than that of the portfolio manager, the upfront payment ("U") is positive on the left side of the equation (the portfolio manager's expected cash flow) and negative on the other side (JPM's expected cash flow) so as to equate the two sides. (As before, "C" designates the coupon payment, "P" is the probability of default, and "V" is the notional value.)
...
This verifies that \$200 is the correct upfront payment for this situation.

That way, with an upfront payment, the two parties can trade with a coupon of 1\% and a default probability of 0.8\%. On January 1, the JPM broker would pay \$200 to the portfolio manager and the portfolio manager would pay \$1,000 to the JPM broker.

Note that in Equation \ref{eqn:Upfront}, if "C" and "V" are known or pre-determined values (which is often the case when people are using that equation to determine cash flows), then you need to know the value "P" to determine "U", and vice versa.  
...
In our simple one-period case, the coupon is 1\%. Another way of expressing this is saying that the spread---equivalent to the coupon, for now---is 100 bps (where 100 bps equals 1\%, 200 bps equals 2\%, etc.). Say that the CDS is traded---given to another protection buyer and protection seller---on March 31, one fourth through the one-year contract. In such a case, the new protection buyer and seller would trade the CDS at a spread of 75 bps, since the seller is only proviving coverage for 3/4 of a year, and thus the new spread need only be 3/4 of the original spread. This serves as an example of how the value of the spread drops from 100 bps on January 1 to 0 bps on December 31. 

This process---the spread drop over the duration of the contract---is known as \textbf{rolling down the curve}. 

Consider another aspect of the CDS: the probability of default. In the simple one-period case, both parties were willing to make a trade that implied a probability of default of 1\%. As the CDS matured, and as the time remaining in the contract decreased, the probability of default (implied by the CDS trade) decreased 1\% on January 1 to 0\% on December 31. On June 30, there was only half of a year remaining during which Alcoa could default (this a 1\%/2 = 0.5\% probability of default) and on December 31, the contract had finished---so a 0\% probability of default. This serves as an example of how there is a direct relationship between probability of default and contract duration. Note that we are referring to the probability of default \emph{within} \emph{the} \emph{context} \emph{of} \emph{this} \emph{specific} \emph{CDS}. 
...
We have introduced a simple one-period CDS between a portfolio manager and a JPM broker and explored four complications that affect the expected cash flows: interest rate, recovery rate, upfront payment, and accrued coupon.
...
Thus far, we have described a simple one-period CDS with one coupon payment on January 1. However, as mentioned in Section \ref{sec:AccruedCoupon}, CDS have four coupon payments each year. Before we get into the exact mechanics of a four-period or more case, consider a simple two-period CDS. This CDS is a two-year agreement and has two coupon payments: one on January 1 of the first year and one on January 1 of the second year. The interest rate is 0\%, and the probability of default is constant for both years of the contract. Similar to the simple one-period CDS, the notional amount is \$100,000 and the coupon is 1\%. Note that the term, "coupon", refers to the annual payment from the protection buyer. So, the buyer will make two \$1,000 coupon payments, or will pay a total of \$2,000 over the two years. Given that both parties agreed to the deal, the probability of default ("P") that both parties were willing to assume is 2\%:
...
Recall that we discussed in Section \ref{sec:OnePeriodOverTime} how the probability of default within the context of a CDS decreases to 0\% as the contract matures. This CDS is no exception. So, after one year of the contract, the probability of default over the second year is 1\%---not 2\%. As such the cash flows for the second year of the contract is as follows:
...
As we concluded, the probability of default, "P", for the second year of the contract is 1\%. Note that the cash flows for the first year of the contract are the same.

----- Stop Here. --------

\subsubsection{Simple Two-Period Case Complications}

Similar to the simple one-period CDS, we made several assumptions in our simple two-period CDS case that are not consistent with the CDS market. Again, we assumed that both parties are risk-neutral.

We are also assuming that we can just simply add the percentages when we calculate the probability of default. 

%% Should we get into pricing here?
...
If the two parties believe that the probability of default will change from year one to year two, then they will have to create a deal that has a corresponding upfront payment. If, for example, they believe the probability of default to be 2\% during year one, and 4\% during year two, then the expected payment from JPM to the portfolio manager would be the expected payments for year one averaged with the expected payment for year two \emph{assuming} \emph{Alcoa} \emph{doesn't} \emph{default} \emph{during} \emph{year} \emph{one}. See calculation below:

\subsubsection{ISDA conventions}
\label{sec:ISDA}
