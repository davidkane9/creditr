...
\Abstract{
 A credit default swap (CDS) is a bilateral agreement between two parties (the protection buyer and the protection seller) with respect to default by a third party. Over the past two decades, CDS have been one of the fastest growing parts of the financial market. First, we explain the basics of CDS along with key concepts like probability of default, coupon, spread, notional, recovery rate, upfronts and the ISDA Standard Model. Second, we introduce Markit and Bloomberg, the two primary sources for CDS data and analysis. Third, we describe the CDS R package, an open source tool which allows users to calculate credit default swap information.
}
...
where U is upfront payment, C is coupon, P is default probability, V is notional amount, RR is recovery rate, i is interest rate. However, in practice to calculate upfront payment is much harder due to complications of non-constant interest rates and default probability. Fortunately, ISDA Standard Model has standardized the calculation of upfront payment for us.
