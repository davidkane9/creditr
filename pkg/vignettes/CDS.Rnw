...
\caption{This is a Bloomberg screenshot that shows the details of an actual CDS transaction on Alcoa Inc priced on June 24, 2014 with 5 years of maturity. The sreenshot consists of three sections: the Deal section (upperleft), Calculator section (bottomleft), and Market section (right side). The Deal section is where traders mostly input detailed information of the trade, such as \textbf{trade date}, \textbf{entity}, \textbf{spread}, \textbf{coupon}, and \textbf{maturity}. The Calculator section shows the calculation results of the CDS contract after the trader has entered the trade information in Deal section. Traders rely on these calculation results to make decisions on trades. The calculation results include \textbf{price}, \textbf{principal amount}, \textbf{Cash Amount}, \textbf{spread DV01}, \textbf{IR DV01}, \textbf{Rec Risk 1\%}, \textbf{Default Exposure}, etc. The Market section shows information of the relevant market rates (such as LIBOR) that we concern as discount factors. This include \textbf{curve date}, \textbf{swap curve}, \textbf{recovery rate}, \textbf{points upfront}, \textbf{default probability} and \textbf{CDS term structure}.}
\label{AlcoaIncCDS}
\end{figure}
...
\caption{This is a figure of Deal section cropped from Figure 2. Traders enter most of the CDS trade information in Deal section, and Calculation section and Market section rely heavily on information provided in ``Deal'' section. Information here include \textbf{notional amount}, \textbf{trade date}, \textbf{maturity}, \textbf{trade spread}, and \textbf{coupon}. \textbf{Conventions} will be set by Bloomberg after the user specifies the \textbf{currency}, \textbf{debt type} and \textbf{restructuring type}. Numerous dates, such as \textbf{first coupon payment date}, \textbf{penultimate coupon payment date}, and \textbf{backstop date}, are also calculated by Bloomberg after the user specifies the trade date and maturity.}
\label{deal}
\end{figure}
...
On upperleft, the CDS contract type is \textbf{SNAC} (\textbf{Standard North American Contract}), which is a convention that specifies how North American single-name CDS are supposed to trade. In European markets, CDS belong to the \textbf{STEC} category, or Standard European Contract.

On bottomright, we see \textbf{Maturity} has two slots. The first slot is the \textbf{tenor}, or length, of the contract. This means that the buyer is protected for 5 years in this case. Tenor can also be denoted in months, e.g. ``5M''. The second slot is \textbf{maturity date}, or \textbf{end date}, of the contract. This refers to when the protection is over. 
...
Then, let's focus on the labels on bottomright: Spread DV01, IR DV01, Recovery Risk (1\%), and Default Exposure. However, to understand these four concepts, we have to introduce \textbf{PV01} first.
...
\caption{
This figure is the ``Market'' section cropped from Figure 2. Market section is another field where traders views information of market curve rates and CDS term structure. \textbf{Curve Date} is the date of LIBOR rates we use to price CDS (see below for important clarifications), and \textbf{Swap Curve} is the yield curve of the swap. \textbf{Recovery rate}, \textbf{points upfront}, \textbf{spread}, and \textbf{default probability} are also listed here. We can also view \textbf{term structure} on the window (X-axis is maturity and Y-axis is spread). The green line with little yellow dots is the ``AA USD Senior Curve'', which means we are buying senior debt protection in USD of Alcoa Inc (stock symbol AA). The big yellow dot is the CDS contract we buy, here at 5-year maturity and 160 basis points of spread. We will also introduce the concepts of \textbf{hazard rates} and \textbf{survival probability} here.}
...
On the screenshot, the ``prob'' label refers to \textbf{default probability}, which is the estimated probability of default for each maturity by a given time. It can be approximated by
...
A CDS term structure is similar to the interest rate curve in that it has the fair spreads of CDS contracts with different maturities. For liquid reference names, these rates are easily available. If the CDS is not liquid, we can use the underlying bond's price to determine the hazard rates. However, if there are only a couple of maturity dates available, we can assume a flat CDS curve.
...
\caption{
This is a screenshot of the online Markit CDS Calculator. It calculates prices for the same CDS trading as the Bloomberg screenshot above.  In this section, we will notice some key terminology differences between Markit and Bloomberg. We will also introduce the Discount Factor, Risky Discount Factor, and CDS indices.
}
\end{figure}
