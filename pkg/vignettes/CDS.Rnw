...
CDSs are used in order to hedge against the \textbf{credit risk} associated with \textbf{bonds or fixed income}. Before we understand CDSs work, we must understand the mechanics of a standard coupon bond, and the risk associated with them.
...
Some of these variables seem self-explanatory. \textbf{REF Entity} (Reference Entity) refers to the name of the company (Alcoa Inc., in this case) that the buyer wants protection against. The \textbf{ticker} is an abbreviated reference symbol for the CDS. Moreover, the \textbf{Trade Date} is the date on which we are making the trade -- June 02, 2014. 
