...
In order to understand this type of exchange, let's first talk about a simpler from of purchasing protection: property insurance. 

Suppose that person owns a \$100,000 house and wants to buy property insurance for the whole house for one year, from January 1 to December 31. For such a purchase, an insurance company charges a fee of \$1,000. Call this \$1,000 the premium. In exchange for this premium payment, the insurance company agrees to monetarily compensate for any property damage for the next year. 

%% For example, if a house fire occurs during that year; then, the insurance company is obligation to pay \$100,000 to the insurance buyer. Otherwise, the insurance company doesn't have to pay anything.

Let's say that the insurance buyer has to pay the premium (\$1,000) at the beginning of the coverage period, January 1. Let's also assume that, in this case of property damage, the insurance company pays out the compensational amount at the end of the year (Decemeber 31) and interest rates are 0\%. Examining the agreement's cash flows, we can see that the insurance buyer pays a \$1,000 premium on January 1 for property insurance for that year, and if, for example, a house fire occurs during that year, the insurance company pays the buyer a compensational amount of \$100,000 on December 31. Note that if there is no property damage that year, then the insurance company pockets the \$1,000 premium payment and doesn't have to pay anything in return. 

As such, before making this deal, both parties would want to consider the probability of property damage for that year. If, for example, the house owner was extremely confident that the house would not be damaged, and thus believed the probability of damage to essentially be 0\%, then he would not want to make this deal. The only cash flow between the two parties would be the \$1,000 premium payment out of \emph{his} pocket for the one-year coverage! Simiarly, if the insurance company thought it was extremely likely that the house would get damaged---maybe it's near a body of water and it's hurricane season---then the insurance company would probably not want to make this deal. In that case, the compensational payment would greatly outweigh the premuim payment, and the insurance company would acquire a net loss of money.

In such a scenario, what probability of property damage would make both parties agree to this deal? In other words, what probability of fire damage would render the \textbf{expected cash flows} for both parties equal? 

To answer this question, we need to first determine the amount that each party expects to pay. The insurance buyer expects to pay a \$1,000 premium on January 1, regardless of whether damage occurs or not. The insurance company's payment, however, is only \$100,000 if damage occurs; otherwise, it's \$0. Thus, if "P" is the probability of damage, we can say the following: 
...
As we can see, the insurance buyer expects to pay the \$1,000 premium and potentially receive the \$100,000 (based on the probability of damage) and the insurance company expects to receive \$1,000 premium and potentially pay the \$100,000 compensation payment. Solving for p, we get:
...
Thus, we can see that if "P" is .01, or 1\%, then this would be considered a fair deal for both parties; the expected cash flows are equal.

Note that the above calculations assume that the compensational amount paid in case of damamge is always equal to the monetary amount the buyer bought insurance for. However, what if, say, a house fire occured and only damaged half of the house? Does the insurance company still have to pay the full value of \$100,000?

Realistically, no. So, when the two parties---insurance buyer and insurance company---initiate the insurance agreement, they need to agree on what kind of damage merits the payment of the full \$100,000, and what doesn't. They would also have to come to a consensus on the definition of "property damage," and how to measure it (how does the insurance company know that there has actually been a fire?). Clearly, this highlights the importance of a contract that specifies the terms of the agreement.

Note also that, even though the house is worth \$100,000, the insurance buyer could choose to buy insurance for only half the house---so only \$50,000 worth of the house. 

\subsection{Simple One-Period Case}
\label{sec:OnePeriod}

Instead of buying insurance against property damage, let's consider buying insurance against a company's inability to meet its obligations. For example, let's say that a portfolio manager owns \$100,000 worth of bonds from Alcoa, a company, and she is concerned that Alcoa won't return the money she lent. In other words, she is concerned that Alcoa will \textbf{default} on its bonds---not pay them back---and that it may even go bankrupt in the next year. So, she enters a one-year CDS agreement with an investment bank---say, J.P. Morgan.
...
The other side of the CDS agreement is known as the \textbf{protection leg} and involves J.P. Morgan, the \textbf{protection seller}. If Alcoa defaults, then J.P. Morgan must pay \$100,000 to the portfolio manager on December 31 of that year to compensate for the loss of her bonds. Note that Alcoa is known as the \textbf{reference entity} in this CDS since it is the company from which the protection buyer desires protection, and the portfolio manager's specific Alcoa bonds are known collectively as the \textbf{reference obligation}.
...
Is this agreement a fair deal? Well, like our property insurance example, it depends on a probability: the \textbf{probability of default}. We can use the following equation to solve for the probability of default, which are now our new "P", that would render this deal fair. The left side represents the cash flows of the portfolio manager, and the right side represents the cash flows of J.P. Morgan:
...
This is the same equation as in the property insurance example! This shouldn't be too surprising, though, since we used the same numerical figures in this example. Thus, we know that if the probability of default is 1\%, then this deal is considered fair.
...
Up until now, we have assumed that the interest rate are 0\% for our CDS example. The \textbf{interest rate} is a benchmark rate that represents the return that investors can expect without much risk. How might the cash flows in the CDS agreement change if the interest rate was \emph{not} 0\%---instead, say 2\%?

First, let's review our CDS agreement: the portfolio manager pays the premium leg on January 1 and J.P. Morgan pays the protection leg (if Alcoa defaults) on December 31. J.P. Morgan receives the \$1,000 coupon a full \emph{year} before the portfolio manager would receive the \$100,000 if Alcoa defaults. In an environment where the interest rate is 2\%, J.P. Morgan could theoretically invest the \$1,000 sum starting January 1 and earn interest for that year:

\begin{equation}
 \begin{aligned}
  {\rm Coupon\hspace{3pt}with\hspace{3pt}Interest = \$1,000 * (1 + .02) = \$1,020} 
    \end{aligned}
\end{equation}

By December 31, the \$1,000 could grow to \$1,020, which gives J.P. Morgan an extra \$20 that the portfolio manager won't receive because she doesn't get a payment---if at all---until the end of the year. As such, the \$1,000 coupon has more value \emph{per} \emph{dollar} than the \$100,000 payment on January 1. If the interest rate is not 0\%, then we need to adjust the cash flows accordingly to account for the extra money that J.P. Morgan gains. Otherwise, the portfolio manager's expected cash flow is larger that that of J.P. Morgan---and why would the portfolio manager want to enter this agreement if it were unfair on its own end?
...
\begin{equation}
 \begin{aligned}
  {\rm Discounted\hspace{3pt}Coupon = \$1,000/(1 + .02) = \$980.39} 
    \end{aligned}
\end{equation}
...
Let's exemplify using our simple one-period CDS: suppose that Alcoa defaults during the one-year CDS contract, but it's able to give the portfolio manager \$50,000 of the \$100,000. In that case, J.P. Morgan would only have to pay the portfolio manager another \$50,000 (not the notional amount of \$100,000) to compensate for her monetary loss from Alcoa's default. The ratio of the amount a bondholder receives after a company's default compared to the amount the bondholder \emph{should} have received (with no default) is known as the \textbf{recovery rate}. So, in the example described above, the recovery rate is 50\%.

Let's assume that, once again, the interest rate is 0\%, and thus not a factor in the expected cash flows for 

%% Like in our property insurance example, we can determine this by looking at the expected cash flows for the portfolio manager and for J.P. Morgan. A fair deal means that these two values are equal. See the generic equation for this simple case below, where "c" is total coupon payment, "d" is the \textbf{probability of default}, "r" is the recovery rate and "n" is the notional amount.
...
%% Before we plug in known values, let's discuss the above equation. The left side of the equation represents the expected payment of the protection buyer, which consists solely of the one coupon payment of \$2,000. The right side of the equation refers to the expected payment of the protection seller, which draws from the same logic as Equation 1 in the property insurance example.

\vspace{-2em}
\begin{equation}
 \begin{aligned}
   {\rm Protection\hspace{3pt}seller's\hspace{3pt}expected\hspace{3pt}payment = (Probability\hspace{3pt}of\hspace{3pt}default)*(Payment\hspace{3pt}in\hspace{3pt}case\hspace{3pt}of\hspace{3pt}default)}
    \end{aligned}
\end{equation}

Note that the only reason Equation BLANK may look more complicated than Equation BLANK is that the "Payment in case of default" has to take in the recovery rate; recall that we had a recovery rate of 50\% that covered \$100,000 of the notional amount, and thus the payment that J.P. Morgan had to make to the portfolio manager was \$100,000.

Now, back to finding the probability of default that gives a fair deal. When we plug in known values, we have the following:

\begin{equation}
 \begin{aligned}
   {\rm \$2,000 = {d} * (({1 - 0.5})*{\$200,000})}
    \end{aligned}
\end{equation}
...
Solving for "d", we get that the probability of default that would make this agreement fair is 2\%.

\subsubsection{Accrued Interest}
\label{sec:AccruedInterest}

We have so far assumed that our CDS agreement between the portfolio manager and J.P. Morgan begins on January 1 and ends on December 31 of the same year. However, what if this agreement wasn't made exactly on January 1? What if it was made halfway through the year, on June 30? Let's assume the probability of default for those six months is indeed 2\%---the probability that renders the CDS contract fair. Under our current description of a CDS agreement, this implies that the portfolio manager would have to pay the same coupon of 1\% (\$2,000) on January 1 for only receiving six months (instead of one year) of protection coverage. Now, the expected payment of the portfolio manager is greater than that of J.P. Morgan.

To account for this, let's first calculate the amount of the coupon that the portfolio manager is unfairly paying---that is, the fraction of the coupon payment that accounts for the first six months of the year (January 1 - June 30), which she does not receive coverage for. This fraction of the coupon payment is known as the \textbf{accrued interest}.

\begin{equation}
 \begin{aligned}
  {\rm Accrued = \frac{180}{360}*\frac{1.0}{100}*\$200,000 = \$1,000}
    \end{aligned}
\end{equation}

Note that we just divided the coupon payment in half, which makes sense since January 1 - June 30 consists of half of the year. Note also that we are dividing 180 by 360 instead of 365 in the above calculation. This particular CDS contract can be said to have a 30/365 \textbf{day count convention}, meaning that the accrued interest calculations are based on the assumptions that there are 30 days in a month and 360 days in a year. This simplication makes calculations much easier. 

%% Why, other than for simplicity's sake, do people use the 30/365 day count convention?

The portfolio manager does not want (and understandably does not have the obligation) to pay for the six months she did not receive coverage for. Instead, it is \emph{J.P. Morgan} who pays the accrued interest to the portfolio manager; after this adjustment, the expected payments from each party are once again equal. Since the portfolio manager is already paying J.P. Morgan a coupon of \$2,000 and now, in this case where the CDS contract initiates on June 30, J.P. Morgan owes the portfolio manager \$1,000 of accrued interest, the two parties can agree to one equivalent cash flow: \$1,000 from the portfolio manager to J.P. Morgan.

Note that accrued interest only becomes relevant when the contract does not initiate on a coupon payment date. In the real-world CDS market, there are four dates each year---instead of, in our example, the one date of January 1---when coupon payments can be made. These dates are known as \textbf{roll dates} and comprise the following: March 20, June 20, September 20, and December 20. As we proceed with our simple CDS example, however, we'll continue with the assumption that January 1 is the one and only "roll date."  
...
So far, we have assumed that the probability of default for our simple, one-period CDS example is indeed 2\%---the probability that renders the CDS contract fair. However, what if the chance of Alcoa's default is \emph{not} 2\%---what if it's 1\%? Clearly, this is no longer a fair deal. The expected payment of the portfolio manager---known as the \textbf{protection buyer} in the CDS agreement---at the end of the year will be greater than J.P. Morgan's expected payment since the probability of default (and thus the probility that J.P. Morgan will have to pay out any money at all) is less than the example above. A simple solution to this issue would be to lower the protection buyer's coupon to 0.6\% (or \$600), as portrayed below (same as Equation BLANK):

%% {\rm \$2,000 = {d} * (({1-r})*{\$200,000})}
...
\begin{equation}
 \begin{aligned}
  {\rm \$2,000 = (.02) * {\$100,000}} 
    \end{aligned}
\end{equation}
When we change the probability of default from 2\% to 1\%, we can adjust the one-time coupon payment accordingly (\$2,000 to \$1,000), as that would be ideal. Note, that we have kept the "Payment in case of default" at \$100,000 because the notional amount is still \$200,000 and the recovery rate is still 50\%.
\begin{equation}
 \begin{aligned}
  {\rm \$1,000 = ({.01}) * {\$100,000}} 
    \end{aligned}
...
However, let's say that, by market convention, all CDS are traded at a standard coupon of 1\%. If the probability of Alcoa's default is less 2\% (1\% in this case), how do we modify the CDS agreement to once again make it a fair trade for both parties? (Note that we are, once again, considered a situation in which the CDS agreement initiates on January 1, not June 30. Thus, accrued interest is not a factor.)

The answer lies in an \textbf{upfront payment}---a payment made from one party to the other at the beginning of a contract (in this case, January 1). If the probability of Alcoa's default is 1\%, then J.P. Morgan has to compensate the protection buyer for paying a coupon of 2\% instead of a coupon of 1\%. Since the difference between the protection buyer's fair payment (\$1,000 at 1\%) and the payment she has to make due to convention (\$2,000 at 2\%) is \$1,000, J.P. Morgan's upfront payment to the protection buyer should be \$1,000. Another way of way of thinking about the upfront payment is to consider the difference in expected payments between the two parties. As before, we want these to be equal, so the party with the smaller expected payment should pay the difference to the party with the larger expected payment. See equation below: 

\begin{equation}
 \begin{aligned}
  {\rm Upfront\hspace{3pt}Payment = {|(Buyer's\hspace{3pt}Expected\hspace{3pt}Payment) - (Seller's\hspace{3pt}Expected\hspace{3pt}Payment)|}}
    \end{aligned}
\end{equation}

\begin{equation}
 \begin{aligned}
  {\rm Upfront\hspace{3pt}Payment = {|(\$2,000) - (.01 * \$100,000)| = \$1,000}}
    \end{aligned}
\end{equation}
...
Up until now, we have described various aspects of a CDS agreement that has one coupon payment on January 1. However, most CDS include more than one coupon payment. Let's explore an agreement in which there are two coupon payments during a two-year contract: one on January 1 of the first year and one on January 1 of the second year. For this case, let's assume that the interest rate (and thus the discount rate) is 0\% and the probability of default is constant for both years of the contract. If the notional amount is still \$200,000, the standardized coupon is still 1\% (so the buyer will make two \$2,000 coupon payments), and the recovery rate is still 50\%, then the probability of default, "d", would have to be 4\% for this to be a fair deal (still relying on Equation BLANK): 

\begin{equation}
 \begin{aligned}
  {\rm \$2,000 + \$2,000 = {(d = .04)}*{\$100,000}} 
    \end{aligned}
\end{equation}
...
For this scenario, there are three basic ways in which the CDS agreement may unfold. Alcoa could default during the first year, Alcoa could default during the second year, or Alcoa could just not default during the two-year contract. 

If the probability of default changes from year one to year two, then this can make our calculation of the upfront payment a bit more complex. If, for example, the probability of default during year one is 4\%, and the probability of default during year two is 5\%, then the expected payment from J.P. Morgan to the portfolio manager would be the expected payments for year one averaged with the expected payment for year two \emph{assuming} \emph{Alcoa} \emph{doesn't} \emph{default} \emph{during} \emph{year} \emph{one}. See calculation below:
