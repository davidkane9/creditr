...
So far we have only discussed pricing of CDS contracts across two periods. Let's say that our Alcoa CDS now has a tenor of five years and that each coupon payment is made quarterly, on the Roll Dates. If we look at Figure \ref{CDSalcoa} below, we can see the details of an actual CDS contract on Alcoa priced on June 23, 2014.
