...
\section{CDS Mechanics}
\label{sec:CDSMechanics}
...
Let's say that a pension fund portfolio manager has to allocate her clients' money (a total sum of \$100 million) such that it earns a steady return without being in a high-risk position. Therefore, this portfolio manager decides to buy several bonds from a company that has a low \textbf{credit risk}---say, Alcoa Inc. A \textbf{bond} is an agreement in which one party lends money to another party in exchange for that same sum of money, plus interest, at a future date. So, the portfolio manager lends money to Alcoa Inc., which she believes will not \textbf{default} on its debt, or render itself unable to pay back its bondholders. A company that has a low probability of default is said to have a low credit risk, and thus be \textbf{credit worthy}.

Since our portfolio manager is risk-averse, she chooses to buy senior debt. Her decision is in line with her interests since a company's bondholders, or debt holders, are compensated before a company's shareholders in the case of default. In other words, if a company defaults on its bonds, the company's remaining assets go first to the bondholders. Furthermore, both bonds and equity can be categorized as "senior" or "junior" (there exist many other classifications, but we'll stick to these two for now). As the terms suggest, senior bonds usually have the greatest seniority in the issuer's capital structure and are repaid in the case of default before junior bonds. 

Such low-risk bonds are classified as \textbf{Investment Grade} (IG). The creditworthiness of a company is often represented by the \textbf{credit rating}---a score determined by rating agencies such as Standard \& Poor's (S\&P) or Moody's. We will not go into the details of how these ratings are determined; we only note that an Investment Grade bond has a rating of BBB- or higher from S\&P or Baa3 or higher from Moody's. Let's say that our portfolio manager's bonds have a maturity of five years and a coupon rate of (TBD). Alcoa Inc. has a credit rating of (TBD) from S\&P.
...
However, six months after the manager bought the bonds, her confidence in the creditworthiness of Alcoa Inc. begins to waiver. In other words, the manager is worried about some \textbf{credit event} related to the company. A credit event can signify the risk of a company's bankruptcy and resulting default, but the term can also expand to include failure to pay out debt within a certain amount of time, a substantial decrease in credit rating or the confiscation of assets, among other events. The company's credit rating or some other indicator of the company's credit risk may incentivize the manager to hedge her position and reduce her portfolio's exposure. 

Of course, she could simply sell the bonds; however, that may be premature or even unprofitable. Instead, she could purchase a financial product that has a value that rises with a drop in the bonds' intrinsic value. In other words, she could take a position that allows her to \emph{profit} from increased credit risk. 

One convenitional technique of doing this is shorting bonds. Shorting is the process of borrowing and selling a security with the obligation to repurchase it in the future and return it to the lender. Clearly, this is profitable if the value of the security drops by the time it is repurchased in the future. However, there are several issues associated with this method. It is difficult to short securities that are not liquid. The portfolio manager may not be able to find enough bonds that will mature at the same time. Moreover, she would have to spend \$100 million to hedge her entire portfolio of \$100 million, which is a substantial opportunity cost.    
...
CDSs were invented in order to resolve the issues that accompany hedging against risk (as described in Section \ref{sec:whyCDSs}). 

As shown in Figure \ref{fig:cashFlow}, a CDS is an agreement between two \textbf{counterparties}, or opposing groups, in which the buyer (in this case, the portfolio manager) pays a fixed periodic coupon, also known as the \textbf{deal spread}, to the seller in exchange for protection against a credit event associated with the buyer's bond.   
...
\subsection{More on CDS Cash Flows and terminology}
\label{CDSCashFlows}

Although the cash flows involved in a CDS seem pretty simple in figure \ref{fig:cashFlow}, the underlying mechanics of a CDS are a bit more complex. To understand this, let us look at the actual CDS data of Alcoa Inc. from Bloomberg\footnote{The two most commonly used sources for CDS related data are Bloomberg and Markit, which we will discuss extensively in a later section.}. Figure \ref{fig:CDSalcoa} below displays many different variables such as RED code, Reference Entity et al. on the CDS of Alcoa Inc. for June 02, 2014. These are standard terms of CDS contracts that were set by the \textbf{International Swaps and Derivatives Association (ISDA)}, the organization that regulates over-the-counter derivatives. 
Let us go into more detail about what these variables are the exactly the cash flow works and what variables our portfolio manager has to consider.   
...
Some of these variables seem rather straighforward. \textbf{Reference Entity} refers to the name of the company for which protection has been bought, which, in this case, is Alcoa Inc. \textbf{Maturity} refers to the \textbf{tenor} or length of the contract. Bloomberg provides the length of the contract or the equivalent maturity date. \textbf{Debt type}, labelled as `Senior', refers to the seniority of the debt.  The \textbf{notional amount} is printed as `10' or \$10 million `USD' (US Dollars). The `MM' between the amount and the currency refers to the type of restructuring of the contract. `MM' stands for 'Modified Modified' Restructuring.   

%% Explain MM

The two terms in this figure that are essential to understanding the cash flow are \textbf{Trd Sprd} (Trade Spread) and \textbf{Coupon}, both given in basis points (bp). In section \ref{CDSbasics}, we stated that the protection buyer pays the protection seller a fixed coupon for purchasing protection. Until 2009, the two counterparties would agree on the coupon level before the trade, and as the market moved, this tradable coupon would vary. In this type of a contract, the present value of the contract (the current worth of a future sum of money) would be zero. 

In April 2009 in North America, and June 2009 in Europe, CDS contracts were standardized such that dealers would have to quote fixed coupons of 100bp or 500bp in North America, or 25bp, 100bp, 500bp or 1000bp in Europe. The \textbf{Coupon} printed in the figure above is the fixed coupon for Alcoa Inc., which is 100bp or 1\% of the notional. However, the dealers may not feel that this is the fair premium for protection. The hedge fund selling protection to the portfolio manager may feel that the fair premium should be 160bp, and not 100bp. This fair premium rate is known as \textbf{Trade Spread} (or just Spread), labelled as 160 in the figure above. 

Naturally, if the hedge fund believes that the fair premium should be 160bp, it would like to compensated for receiving just 100bp in advance. The portfolio manager would have to make what is known as an \textbf{Upfront Payment}, labelled as \textbf{Cash Amount} in Figure \ref{fig:CDSalcoa}. With the information for Alcoa given in the figure, this amount is \$258478. Therefore, the total payments made by the portfolio manager over the life of the contract would look like Figure \ref{fig:CDSnoevent}. 

%% Need to re-format lines here.
...
Holding a portfolio of a disproportionate number of Alcoa Inc. bonds is naturally a risky idea. In the real world, most successful 
...
our portfolio manager decides to reduce her risk by holding bonds of 
...
We compared the results from our package with results from Bloomberg and Markit. Interestingly,
...
\begin{figure}[H]
\centering
\includegraphics[width=2.2in, height=4.5in]{images/MarkitIRJun2.png}
\caption{Interest Rate figures from Markit.com}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=3.5in, height=3in]{images/BloombergIRJun2.png}
