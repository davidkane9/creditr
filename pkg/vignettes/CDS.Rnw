...
Then, let's focus on the labels at the lower right: Spread DV01, IR DV01, Recovery Risk (1\%), and Default Exposure. However, to understand these four concepts, we have to introduce \textbf{PV01} first.
...
On the screenshot, the ``prob'' label refers to \textbf{default probability}, which is the estimated probability of default for each maturity by a given time. If we assume that 
