...
In this section, we will explain CDS contract in software that traders often use: Bloomberg and Markit CDS Calculator.

\subsection{Bloomberg Overview}
The figure below shows the details of an actual CDS contract on Alcoa priced on June 24, 2014.

\begin{figure}[H]
\centering
\includegraphics[width=6.3in, height=3.8in]{images/AlcoaIncCDS.jpg}
\caption{This is a Bloomberg screenshot that shows a CDS transaction for Alcoa. There are three sections: the Deal section (upperleft), Calculator section (bottomleft), and Market section (right side). The Deal section contains the details of the CDS trade, such as trade date, entity, spread, coupon, and maturity, etc. The Calculator section shows the calculation results of the CDS contract, such as Price, Principal, ConUpfront, and Spread DV01, etc. The Market section shows CDS term structure and default probability.}
\label{AlcoaIncCDS}
\end{figure}
