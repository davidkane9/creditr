...
3. In this example, we also assume that the interest rate is constant over the two years, but that's far from the reality. In real life, the interest rates we use for CDS pricing often change, so how should we calculate the discount factor and re-do CDS pricing?
