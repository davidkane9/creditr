...
The risk free rates are an essential component in the pricing of CDSs. They are used to determine the present value of future cash flows and to calculate the probability of default (explained later). If we look at Figure \ref{fig:YieldCurve}, we can see the yields for different treasury bonds, at 8pm UTC, June 23 2014.     
...
In calculating the price of a CDS we use the yield curve of the \textit{previous day}, and use the rates for the corresponding currency. So for CDSs denominated in USD, we will use the US treasury rates, for those in GBP we will use UK Government bond yields and so on.  
