...
The Standard Model assumes that the mark-to-market value is computed by discounting the expected protection leg and premium leg cashflows to the trade date. What this basically means is that the price of a CDS is the sum of the present value of the premium leg and protection leg. 
...
At this point it is important to understand \textbf{PV01} or \textbf{present value 01} since it used to calculate the cash flows and risk measures of a CDS. PV01 is the present value of a stream of 1 basis point payments at each CDS coupon date. It is sometimes referred to as the \textbf{CDS duration} or \textbf{risky duration}. Analytically, PV01 can be calculated by
...
The present value of the premium leg is the present value of future coupon payments. It is simply coupon payment mutliplied by the PV01
