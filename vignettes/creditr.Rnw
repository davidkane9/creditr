...
In this section, we will demonstrate the use of the \textbf{creditr} package in detail and provide a series of examples. The \textbf{creditr} package implements the ISDA Standard Model, allowing users to value credit default swaps and calculate various risk measures associated with these instruments. 
