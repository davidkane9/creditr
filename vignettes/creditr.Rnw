...
Note that Highbridge's expected cash flows are the same as that of the homeowner in the property insurance example, and Citigroup's expected cash flows are the same as that of the insurance company. \ref{eqn:portfolioECF} corresponds with \ref{eqn:homeownerECF} while \label{eqn:CitigroupECF} corresponds with \ref{eqn:companyECF} from our property insurance example. This shouldn't be too surprising, though, since the numerical figures in both examples are the same. Also, note that the probability estimates serve exactly the same roles as before: $P_h$ and $P_c$ are the default probability estimates for the homeowner/Highbridge and for the insurance company/Citigroup.
...
Assume that a deal is reached; both parties agree to the CDS. As in the property insurance example, we can equate the cash flows to find the \textbf{risk-neutral probability of default}, which is the value of $P$ implied by the deal. Note that an alternate definition of this probability is the value of $P$ at which the premium leg equals the protection leg. However, we must remember that the risk-neutral probability of default is a mathematical concept that bears no necessary connection with the true probability of default, the protection buyer's estimate of the probability of default, or the protection seller's estimate of the probability of default. Investors can never know the true probability of default, so credit default swaps are all about which party makes a more accurate guess of $P$. In the following equation, $C$ represents the coupon ad $V$ represents the notional.
...
Consider what happens to the simple one-period CDS over the life of its contract---specifically, how the risk-neutral probability of default (or risk-neutral $P$), the \textbf{mark-to-market} value, the \textbf{spread}, and the \textbf{profits and losses} (P\&L) of the Highbridge portfolio manager and Citigroup change from January 1 to December 31, assuming that Alcoa does not default during that year. Table 1 shows the evolution of these values.

The risk-neutral probability of default is the probability of default which equates the cash flows of the protection buyer and the protection seller in a CDS agreement. $P$ does not necessarily match up with the true probability default nor either party's estimate of the probability of default. Risk-neutral $P$ is simply a mathematical concept. In this subsection, we will discuss two different types of risk-neutral probabilities of default: $P$ and $P_{cum}$. $P$ represents the annualized probability of default, or the overall probability of default during one period of the CDS contract. Right now, since we only have one period in our entire contract, $P$ will remain constant. However as we get into CDS with more periods, $P$ can potentially be different for each period. $P_{cum}$ is the cumulative risk-neutral probability of default. In other words, it is the risk that the reference entity will be unable to repay bondholders between a specific date and the end of the contract.   

We can think of the \textbf{mark-to-market} value as the price at which the CDS would sell at any given time over the life of the contract. Investors often trade CDS in between the start and end dates of the contract, so the mark-to-market value determines changing the price of the CDS as time goes on. The mark-to-market changes throughout the life of a CDS because, as time progresses, the financial health of the company changes. Also, the closer we get to the end of the CDS, the less time the reference entity has to default. According to the table, on January 1st, the Highbridge investor pays \$1,000 to Citigroup for protection. She and Citigroup agree that \$1,000 is an appropriate price for a year of protection against Alcoa's default. Let's say that on March 31st, a second investor wants to purchase the contract from Highbridge. Yet since Alcoa's financial health remains just as good as in January and Alcoa has less time to default, they have a lower probability of default. With a smaller likelihood of receiving the protection payment then Highbridge had, the second investor demands a lower price than \$1,000. Highbridge and the second investor conclude that the contract is worth \$750 (the mark-to-market value) because only 75\% of the time in the contract is left.  

We can consider the \textbf{spread} to be equal to the coupon in this particular CDS. Think of the spread as the cost of protection written as a portion of the notional amount (the amount on which the buyer purchases protection). Like the mark-to-market value, the spread reflects the likelihood of the reference entity's default. In Table 1, note that as the probability of default decreases, the spread decreases and it costs less to purchase protection. One difference between coupons and spreads is that we refer to coupons in percentages---in this case, 1\%---and we refer to spreads in basis points (bps). 1\% is equal to 100 bps. In our simple one-period case, we say that the coupon is 1\% and the spread is 100 bps. When talking about the worth of a CDS contract, investors most frequently talk in terms of spread rather than dollar values or percentages. People prefer to use spreads because basis point make it easier to compare CDS contracts with different reference entities and notional amounts. For example, an investor could pay \$1,000 for protection on \$10,000 of one reference entity and also pay \$1,000 for protection on \$1,000,000 of a different reference entity. If he discussed these two contracts in terms of the coupon in dollars, the contracts would appear to be equal. Yet speaking in terms of basis points, we can see that, the first reference entity actually has a much higher risk of defaulting on its bonds than the second reference entity. Basis points are also more convenient for discussing CDS with coupons less than 1\% because basis points are smaller. However, we will soon learn that the real-world CDS must be traded at coupons 1\% or 5\%. Therefore, the spread represents what the CDS would be traded at in the absence of a fixed coupon. The spread it the rate that both parties agree on as a fair price for protection. We can observe in Table \ref{table.1} that the spread decreases from 100 bps to 0 bps over the year---a reflection of the fact that, at the end of the contract, Citigroup has earned and been paid the full \$1,000 coupon payment by providing a full year of coverage. This process---the spread drop over the duration of the contract---is known as \textbf{rolling down the curve}. 

P\&L, or \textbf{Profits \& Losses}, represents the change in market value of the CDS contract on, in this case, a day to day basis. A good way of thinking about the P\&L of, for example, Citigroup in this CDS is to consider the fraction of the coupon payment that Citigroup \emph{earns} each day. We know that Citigroup receives a coupon payment of \$1,000 on January 1, but Citigroup's P\&L measures what Citigroup earns each day that it provides protection against Alcoa's default during that year. Citigroup can only be certain that they have made the \$1,000 on December 31st because by that point they know that Alcoa did not default during the contract and they will not have to make the protection payment. On June 30, for example, they know for sure that they have gained half of the coupon because half of the CDS duration has passed with Alcoa's default.

Table 1 displays the evolution of the above-mentioned variables as the contract matures from January 1 to December 31. Note that we are considering the case in which Alcoa does not default during the one-year contract.
...
\begin{tabular}{lllllllll}
  \hline
Date & $CF_H$ & $CF_C$ & $P$ & $P_{cum}$ & MTM & Spread & $P\&L_H$ & $P\&L_C$ \\ 
  \hline
  Jan 1 & -\$1,000 & \$1,000 & 1\% & 1\% & \$1,000 & 100 bps & \$0 & \$0 \\ 
  Mar 31 & \$0 & \$0 & 1\% & .75\% & \$750 & 75 bps & -\$250 & \$250 \\ 
  Jun 30 & \$0 & \$0 & 1\% & .50\% & \$500 & 50 bps & -\$500 & \$500 \\ 
  Dec 31 & \$0 & \$0 & 1\% & 0\% & \$0 & 0 bps & -\$1,000 & \$1,000 \\
   \hline
\end{tabular}
}

\caption{This table measures how several variables --- the cash flows (CF) for each party, the risk-neutral value of $P$ and $P_{cum}$, the mark-to-market (MTM) value of the contract, the spread, and the profit and loss (P\&L) for each party --- change as the simple one-period CDS matures. Since the reference entity (Alcoa) does not default, the only cash flow is the \$1,000 coupon payment from Highbridge to Citigroup on January 1. The risk-neutral value of $P_{cum}$ decreases from 1\% to 0\% as the contract matures. Note that this ``linear'' drop off in the risk-neutral $P_{cum}$ is proportional to the time remaining until the termination of the contract. For more realistic contracts, the relationship is non-linear. $P$ remains constant at 1\% because it is the overall risk-neutral probability of default for one period of CDS coverage. The mark-to-market value decreases from \$1,000 to \$0 from January 1 to December 31 and the spread decreases from 100 bps to 0 bps. Note that as Citigroup profits from providing protection coverage (gains \$1,000 by Dec 31), Highbridge loses. On January 1, the P\&L of both parties is 0.} 
\label{table.1}
\end{table}

Table \ref{table.1} shows that the \textbf{risk-neutral $P$} decreases from 1\% on January 1 to 0\% on December 31. In this situation during which Alcoa does not default, the company remains just as healthy so investors can be more certain that the protection payment will not be made. Alcoa also has fewer days left to default the closer we get to December 31. On June 30, $P$ has dropped to 0.5\% because only half of a year remains for Alcoa to default during the contract---and thus $P$ is half of its initial value. This demonstrates the direct relationship between the risk-neutral $P$ and the contract duration.

\textbf{Cash flows} show the gain or loss on a specific day for each side of the CDS contract. It is important to understand how cashflows differ from Profits \& Losses. Cash flows strictly show the amount of money that changed hands on the exact day that that cash was exchanged. P\&L is more of a theoretical concept, representing when payments are certain and unable to be cancelled out by other payments. Cash flows, on the other hand, are a concrete record of actual payments. 
...
The next five subsections address interest rate, recovery rate, accrued coupon, non-standard coupon, and upfront payments. These complications affect real-world CDS agreements.
...
First, look at our CDS agreement: Highbridge pays the premium on January 1 and Citigroup pays the \$100,000 (if Alcoa defaults) on December 31. Citigroup receives the \$1,000 coupon a full \emph{year} before Highbridge would receive the \$100,000 payment if Alcoa defaults. In an environment where the interest rate is 20\%, Citigroup could theoretically invest the \$1,000 sum starting January 1 and earn interest for that year.

The spread is paid at the start of the contract and the protection payment is made at the end, and the value of money decreases over time. In our example, the notional is \$100,000. Yet to find how much \$100,000 on December 31 would be worth on January 1 of the same year, we can discount by an interest rate. Protection buyers must be careful of interest because the notional amount they agree on for on day one of the contract will be worth less than the payment they receive at the end if Alcoa defaults.
...
\begin{equation}
 \begin{aligned}
  {\rm -\hspace{3pt}\$1,000 + \frac{P \times \$100,000}{(1 + 0.2)} = \$1,000} - \frac{P \times \$100,000}{(1 + 0.2)} 
    \end{aligned}
\end{equation}

Solving for $P$, we get 1.2\%. Since both parties agreed to this CDS in an environment where the interest rate is 20\%, we can say that Highbridge's prediction for the probability of default is at least 1.2\%, and Citigroup's prediction for the probability of default is at most 1.2\%. Notice that the probability of default is larger once we discount the notional amount. The investment manager at Highbridge must accept a smaller protection payment if it is more likely that she will receive the payment. On the other side, as the risk that Citigroup will have to pay the notional amount gets higher, they will wager a smaller protection payment because they are more nervous that they will have to pay it. The balance of these two positions mean that risk-neutral probability of default and the notional amount move in opposite directions. To sum up this concept, the higher the interest rate, the lower the discounted value of the eventual payout. The lower the future payout, the higher the probability of default. Remember that these statements only apply in situations like our example where we have a fixed coupon rate. 

\subsubsection{Recovery Rate}

Recall that in our property insurance example, we mentioned that some cases---such as a house fire that burns half of a house---may not merit a full payment equal to the notional amount (in that case, \$100,000). Similarly, when the reference entity defaults, the company still retains a portion of its former value. In future sections, we will discuss how this value is retained, but for now we will focus more on the effect of recovery on the protection leg of a CDS.

The \textbf{recovery rate} is the percentage of remaining assets after a company defaults. Imagine that Alcoa defaults in our example CDS, but they maintain 55\% of their assets. In this case, the recovery rate would be 55\% and Citigroup would only have to pay 45\% of the notional amount. Consequently, the Highbridge PM would receive \$45,000 rather than \$100,000 in the event of Alcoa's default.

Therefore, we need to factor the recovery rate into our calculation of expected cash flows because it changes the value of the protection leg in the simple one-period CDS. Since Highbridge and Citigroup will only both agree to this CDS if the expected cash flows are equal, we set Highbridge's expected cash flows and Citigroup's expected cash flows equal to each other. ($RR$ refers to the recovery rate and, as before, $P$ is the risk-neutral probability of default, $C$ is the coupon payment, and $V$ is the notional value.)
...
Notice that the $P$ increases once we take recovery rate into account. When you take away the recovery rate from the notional amount, we are left with a much smaller protection payment for the same coupon rate. The protection buyer will only accept this smaller sum if she has a higher chance she will actually receive the payment. She would not purchase protection on \$45,000 for a coupon that is equal to protection on \$100,000 unless there is a greater chance she would receive the \$45,000 than the \$100,000. As a general rule for CDS contracts with the same coupons, as the recovery rate increases and the protection payment gets smaller, the risk-neutral probability of default increases. As the recovery rate decreases leading to a greater protection payment, the risk-neutral probability of default gets lower.


\subsubsection{Accrued Coupon}
\label{sec:AccruedCoupon}

Back to our simple one-period CDS example---0\% interest rate, 0\% recovery rate. For this scenario, both the premium leg and the protection leg are paid on December 31. We have so far assumed that our CDS agreement between Highbridge and Citigroup begins on January 1 and ends on December 31 of the same year. However, what if this agreement wasn't made exactly on January 1? What if it was made on March 31, one quarter through the year? Using our simple one-period CDS example, this implies that Highbridge would have to pay the same coupon of $1\% \times (\$1,000)$ on March 31 for only receiving nine months (instead of one year) of protection coverage. Note that so far we have only talked about \$1,000 coupons. However, this is not always the case. Coupons are a function of the protection buyer and protection seller's joint estimate of the reference entity's fiscal health. For example, Highbridge and Citigroup could believe that a fair coupon would be 3\% or 300 bps of the notional amount. But let's return to our familiar \$1,000 and 1\% coupon for now.

Assume that both parties agreed to a deal under these conditions. 

Since the risk-neutral probability of default decreases as the contract duration decreases (see Table \ref{table.1}), the risk-neutral probability that Alcoa will default in nine months is less than the risk-neutral probability that it will default in twelve. As such, the expected cash flows are no longer equal and therefore the two parties must have accounted for this discrepancy---otherwise Highbridge would not have agreed to the above CDS because Highbridge would be paying for the most uncertain part of the protection when this part of the contract has already gone by. In other words, given that the agreement was made, Citigroup must have paid an additional sum to Highbridge to compensate for the fact that she is paying the coupon for one-year coverage and is only receiving nine months.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Find a place for this in the Bloomberg Section
Note that we are dividing 90 by 360 instead of 365 in the above calculation. This is because most CDS contracts assume a year with 360 days instead of 365. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Therefore, the risk-neutral probability of default in this CDS is .75\%. This value is lower than the 1\% probability we observed in \ref{table.1}. Recall that we observed that the risk-neutral probability of default decreases as the contract matures. 
...
\begin{tabular}{lllllll}
  \hline
Date & Mark-to-market & $P\&L_H$ & $P\&L_C$ & Accrued Coupon & $cf_H$ & $cf_C$\\ 
  \hline
  Jan 1 & \$1,000 & \$0 & \$0 & \$0 & \$0 & \$0\\ 
  Mar 31 & \$750 & -\$250 & \$250 & \$250 & \$0 & \$0\\ 
  Jun 30 & \$500 & -\$500 & \$500 & \$500 & \$0 & \$0\\ 
  Dec 31 & \$0 & -\$1,000 & \$1,000 & \$1,000 & -\$1,000 & \$1,000\\
   \hline
\end{tabular}
}
...
The accrued coupon increases as the contract matures---or, in other words, the coupon which accounts for the time in the contract that has passed \emph{increases} as the contract matures.

Let's say that Citigroup and Highbridge want to make a deal on March 31 of the contract described above. The Highbridge portfolio manager will not agree to the deal if she has to unfairly pay for the quarter of the year for which she was not protected (\$1,000 is the price for 1 year of protection, not 9 months). However, ISDA rules say that CDS must be traded at coupons of 1\% or 5\% of the notional amount. Then, the accrued coupon can keep the deal fair to both parties and within ISDA regulations. Highbridge can still pay a \$1,000 coupon, but Citigroup will pay the accrued coupon of \$250 to Highbridge. This amount is on fourth of the notional amount to compensate for the fourth of the year before Highbridge was covered.
...
Imagine that our reference entity, Alcoa, defaults on June 30, the halfway point of the contract. In the event of default, Citigroup would pay the notional to Highbridge and then the CDS protection would be over. Since Highbridge is no longer protected after default, they only have to pay \$500 for the first half of the year that they were protected. The partial coupon payment of \$500 is known as the accrued amount.
...
With an upfront payment, the two parties can trade with a coupon of 1\% and a risk-neutral probability of default of 3\%. On January 1, Highbridge would pay \$2,000 of the upfront payment to Citigroup, and then Highbridge would make \$1,000 coupon payment to Citigroup on December 31.
...
\begin{tabular}{llllll}
  \hline
Date & $P$ & Mark-to-market & Upfront Payment & $cf_H$ & $cf_C$\\ 
  \hline
  Jan 1 & 3\% & \$1,000 & \$2,000 & -\$2,000 & \$2,000\\ 
  Mar 31 & 2.25\% & \$750 & \$1,500 & \$0 & \$0\\ 
  Jun 30 & 1.5\% & \$500 & \$1,000 & \$0 & \$0\\ 
  Dec 31 & 0\% & \$0 & \$0 & -\$1,000 & \$1,000\\
   \hline
\end{tabular}
}
...
In the real world, however, figuring out the upfront payment is not so simple. Complications such as changing interest rates mean that the value of a dollar that changes hands during the upfront payment is not equal to the value of a dollar that changes hands later on in during the coupon payments. Once the protection buyer and the protection seller agree on a fair spread for a CDS contract, they can plug the details of the CDS into the ISDA Standard model to determine the upfront payment. This model is a convention for turning a spread into cash. These cash values are divided into an upfront payment and a 1\% or 5\% coupon rate.
The ISDA Standard Model provides a curve to predict the interest rates at any time in the reasonable future. Certain organizations and big banks publish suggested interest rates for specific maturities. ISDA fills in the gaps between these maturities in case a CDS falls in between. The equations for the ISDA Standard Model are written in the coding language, C. In order to make their predictions, ISDA make assumptions that are close to what will happen in the real world, but will probably be a little off of the values that will really be true in the future. One example recovery rate, which ISDA assumes to be 40\%. However, reference entities can actually retain slightly more or less of their assets after default. But while investors cannot take ISDA predictions to be perfect predictions of a CDS contract, the ISDA Standard Model is widely used and provides very close predictions. For transparency and accessibility, the source code for the ISDA Standard Model is freely available. (see cdsmodel.com or White, 2013 for more info on the ISDA Standard Model)

%% Put the paragraph below in the N-period section
...
We have introduced a simple one-period CDS between a Highbridge portfolio manager and Citigroup, and we have explored five complications that affect the expected cash flows: interest rate, recovery rate, accrued coupon, non-standard coupon and upfront payment.
...
\begin{align}
  \rm Highbridge\hspace{3pt}Portfolio\hspace{3pt}manager's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows &= \rm Citigroup's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows \\
  \rm Premium\hspace{3pt}leg &= \rm Protection\hspace{3pt}leg \\
  \rm U -\hspace{3pt}C + \frac{P \times V \times (1 - RR)}{(1 + i)}  &= \rm -\hspace{3pt}U + C - \frac{P \times V \times (1 - RR)}{(1 + i)}
\end{align}

Note that for any standard CDS contract, the two unknown variables are $P$ and $U$. $C$, the coupon rate, is fixed. The notional amount is agreed upon by the two parties. When using the ISDA Standard Model, the recovery rate, $RR$, is fixed at \%40. This leaves two unknown variables: $P$ and $U$, If we know $P$, we can calculate $U$ and vice versa. Assume a coupon of $1\%$, an effective interest rate of $0.1$, a risk-neutral $P$ of $1.5\%$, and a recovery rate of $0.55$: \\
...
Thus far, we have described a simple one-period CDS with one coupon payment. In reality, CDS have four coupon payments each year. These quarterly dates fall on March 20th, June 20th, September 20th, and December 20th, and they are known as \textbf{IMM dates or roll dates}. Before we get into the exact mechanics of a four-period or more case, consider a simple two-period CDS that has a duration of two years and two coupon payments: one on January 1 of year one and one on January 1 of year two. The interest rate is 0\%, and the probability of default is constant for both years of the contract. Like the simple one-period CDS, the notional amount is \$100,000 and the coupon is 1\%. Note that the term ''coupon'' refers to the protection buyer's \emph{annual} payment. So, the buyer will make two \$1,000 coupon payments, thus paying a total of \$2,000 over the two years. The protection buyer is still Highbridge and the protection seller is still Citigroup. Assume that both parties agree to this deal. 

The risk-neutral probability of default---the default probability that the deal implies both parties were willing to compromise on---can be calculated by equating the expected cash flows. Consider the expected cash flows of the CDS contract.

\begin{align}
  \rm Highbridge\hspace{3pt}Portfolio\hspace{3pt}manager's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows &= \rm -(\hspace{3pt}\$1,000 + \$1,000) + (P \times {\$100,000}) \\
...
The risk-neutral probability of default for the entirety of the two-year contract is 2\%.

Recall how we discussed in Section \ref{sec:OnePeriodOverTime} that the probability of default within the context of a CDS decreases to 0\% as the contract matures.

Consider \ref{table.4}.
\begin{table}[H]
\centering
{\footnotesize
...
Date & $CF_H$ & $CF_C$ & $P$ & MTM & Spread & $P\&L_H$ & $P\&L_C$ \\ 
  \hline
  Jan 1, $Y_a$ & \$0 & \$0 & 2\% & \$0 & 200 bps & \$0 & \$0 \\ 
  Mar 31, $Y_a$ & \$0 & \$0 & 1.75\% & -\$250 & 175 bps & -\$250 & \$250 \\ 
  Jun 30, $Y_a$ & \$0 & \$0 & 1.50\% & -\$500 & 150 bps & -\$500 & \$500 \\ 
  Dec 31, $Y_a$ & -\$1,000 & \$1,000 & 1\% & \$0 & 100 bps & -\$1,000 & \$1,000 \\ 
  Mar 31, $Y_b$ & \$0 & \$0 & .75\% & -\$250 & 75 bps & -\$1,250 & \$1,250 \\ 
  Jun 30, $Y_b$ & \$0 & \$0 & .50\% & -\$500 & 50 bps & -\$1,500 & \$1,500 \\ 
  Dec 31, $Y_b$ & -\$1,000 & \$1,000 & 0\% & \$0 & 0 bps & -\$2,000 & \$2,000 \\
   \hline
\end{tabular}
}
...
\label{table.4}
\end{table}
Since the contract spans 2 year, we can see that Highbridge makes the coupon payment at the end of each year to get protection for both years. Since Alcoa does not default, Highbridge has a net cashflow of -\$2,000 while Citigroup makes a total of \$2,000. The probability of default decreases linearly from \%2 to \%0. This declining risk occurs because Alcoa has fewer days to default and investors have less uncertainty about Alcoa's financial health as we approach December 31 of the second year. The mark-to-market value declines for the first year because as we get more confident that Alcoa will not default, the CDS contract loses worth. For example, if a portfolio manager from AQR wanted to buy the CDS from Highbridge on March 31 of the first year, AQR would pay \$750 (three-quarters of the coupon payment) to Highbridge for the contract since three-quarters of the year remains. Since Highbridge paid \$1,000 for the contract and only made \$750, they have a net profit of -\$250, which matches the mark-to-market for the day they sold contract to AQR. The spread column displays the total amount that a portfolio manager would have to pay for protection if she owned that CDS from a specific date until the end of the contract. For example, if AQR bought the CDS from Highbridge on June 30th of the first year, AQR would have to pay 150 bps: 50 bps for protection for half of the first year plus the full coupon of 100 bps for protection for the second year. The Profits \& Losses columns divide the cashflows proportionately throughout the year to show how much of the coupon each company has gained or lost on a day to day basis.

\subsubsection{Non-constant Probability of Default}
\label{sec:nonconstprob}

In the simple two-period CDS and in \ref{table.4}, we assumed that the overall risk-neutral  probability of default remained the same throughout the entire life of the CDS. $P$ was conveniently 2\%, meaning each year had a probability of default of 1\%. Yet in reality, the fiscal health of a reference entity is always changing, and that company has different sized bond payments due each year. For example, in our two-period CDS situation, if Alcoa had \$10,000 bond due in the first period and a \$1,000,000 bond due in the second period, they would have a higher probability of default in period two. Let $P_1$ an $P_2$ be the probabilities of default in the first and second periods respectively. Our overall risk-neutral $P$ occurs when $P_1$ and $P_2$ are equal. However, the protection buyer and the protection seller can each have different estimates of $P_1$ and $P_2$ and come to a deal if the estimates fit within the same overall $P$. The cashflow equations below relate $P_1$ and $P_2$ for a two period CDS with a different probability of default for each period. We will stick with the \$100,000 notional amount and \$1,000 coupon from previous examples. In order to ensure that Alcoa does not default in the second period if they defaulted in the first period, we multiply the protection payment by $(1-P_1)$.

\begin{align}
   \rm Highbridge\hspace{3pt}PM's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows &= \rm Citigroup's\hspace{3pt}expected\hspace{3pt}cash\hspace{3pt}flows \\
    \rm -\$2,000 + (1-P_1)(\$100,000\times P_2) + P_1\times \$100,000 &= \rm \hspace{3pt}\$2,000 - (1-P_1)(\$100,000\times P_2) - P_1\times \$100,000\\
    %% This equation is a little long and goes into the margin is that ok?
    \rm -2 + (1-P_1)(100\times P_2) + P_1\times 100 &= \rm \hspace{3pt}2 - (1-P_1)(100\times P_2) - P_1 \times 100\\
    \rm (1-P_1)(200\times P_2) + (P_1\times 200)&= \rm \hspace{3pt}4 \\
    \rm (1-P_1)(P_2) + P_1 &= \rm \hspace{3pt}\frac{4}{200} \\
    \rm (1-P_1)(P_2) + P_1 &= \rm \hspace{3pt}2\%
\end{align}

Therefore, $P_1 = 2\%$, $P_2 = 0\%$ and $P_1 = 0\%$, $P_2 = 2\%$ are both examples of probability pairs that that would equate Citigroup and Highbridge's cashflows. For example, the two parties could still come to a deal even if the Highbridge PM thought that Alcoa had a 2\% chance of default in the first period and a 0\% chance of default in the second. She could still reach a deal with Citigroup if Citigroup thought that Alcoa had a 0\% chance of default in the first half of the contract and a 2\% chance of default in the second.
However, the example above still too simple because the probability of default can change within a period. Recall \ref{table.4}. Let's look at the effect of non-constant probability of on the spread of the CDS. Remember that the annual probability of default, $P$, represents the overall probability of default for on year. The cumulative probability of default, $P_{cum}$, displays the total probability of default from a given date until the end of the contract. For the following table, we are disregarding the effects of interest and recovery rate.
...
\begin{tabular}{llll}
  \hline
Date & $P$ & $P_{cum}$ & Spread \\ 
  \hline
  Jan 1,  1\% & 6\% & 600 bps \\ 
  Mar 31, 1\% & 5.75\% & 575 bps \\ 
  Jun 30, 1\% & 5.50\% & 550 bps \\ 
  Jan 1,  5\% & 5\% & 500 bps \\ 
  Mar 31, 5\% & 3.75\% & 375 bps \\ 
  Jun 30, 5\% & 2.50\% & 250 bps \\ 
  Dec 31, 5\% & 0\% & 0 bps\\
   \hline
\end{tabular}
}
\caption{This table shows a CDS contract with a non-constant probability of default. The table also portrays the effect of probability on spread over the life of of this two year, two period CDS. The annual probability of default is 1\% for year one and 5\% year two. The cumulative probability of default decreases linearly by the year as we get closer to the realization that Alcoa will not default. The spread matches up with $P_{cum}$ because the spread is the total worth of the protection from a specific date at the end. Remember that the spread is the most common way that investors compare CDS and the spread is rate at which CDS would be traded in the absence of fixed coupon rates.}
\label{table.nonconstprob}
\end{table}

\ref{table.nonconstprob} shows a two period CDS over a two year duration. The contract has an annual probability of default is 1\% for year one and 5\% year two. These two numbers result an initial cumulative probability of default of 6\%. $P_{cum}$ decreases linearly by the respective annual probability of default for each year. The spread, or desired payment for protection, aligns with $P_{cum}$. Recall that portfolio managers and investment banks most frequently compare CDS using spread.

\subsubsection{More Default Probabilities}
\label{sec:PconstantandPisda}
If we do not have all the information \ref{table.nonconstprob} the following two equations can be used to calculate annual probability of default, $P$, or the spread, $S$ (which can easily lead us to $P_{cum}$). Let $RR$ be recovery rate and let $t$ be the time in years until maturity. Note that these two equations are the inverses of each other:

\begin{align}
    \rm P &= \rm \hspace{3pt}1 - e^{-S \times \frac{t}{1-RR}} \\
    \rm S &= \rm \hspace{3pt}ln(1-P)\times \frac{R-1}{t}
\end{align}

These equations allow us to find to more types of probability of default: the constant probability of default and the ISDA probability of default.
Imagine a CDS agreement with multiple probabilities of default for multiple periods. If we know the spread, duration, and recovery rate of a CDS we can plug in these values to the above equations and get $P_{constant}$. \textbf{$P_{constant}$} is the risk neutral probability (the cashflows must be equal) where all of the underlying probabilities are equal. For example, if the protection buyer believed $P$ to be 2\% for the first of two periods of a CDS contract and 0\% for the second period, $P_{constant}$ would be 1\%. The following example uses the same coupon and notional as \ref{sec:nonconstprob}:

\begin{align}
      \rm -\$2,000 + (1-P_1)(\$100,000\times P_2) + P_1\times \$100,000 &= \\
      \rm \hspace{3pt}-\$2,000 + (1-P_{constant})(\$100,000\times P_{constant}) + P_{constant}\times \$100,000\\
      \rm -\$2,000 + (1-.02)(\$100,000\times 0) + .02\times \$100,000 &= \\
      \rm \hspace{3pt}-\$2,000 + (1-.01)(\$100,000\times .01) + .01\times \$100,000
\end{align}

The equations in this section can also be used to find the ISDA standard model estimated probability of default. The automatically selected ISDA recovery rate is 40\%. Recall that recovery rate means that the reference entity can still retain value after default. Therefore, the protection seller can eliminate the recovered assets from the notional and have a smaller protection payout. When the protection payment decreases, the risk neutral probability of default increases because the protection buyer does not want to take as big a risk when it is more likely that they will lose money. Consequently, the ISDA probability of default will be greater than probabilities that do not take recovery rate into account. By plugging in 0.40 for RR, we can find \textbf{$P_{ISDA}$}. Let's use a familiar example from the previous section with a spread of 600 bps and a time frame of 1 year:

\begin{align}
    \rm P_{ISDA} &= \rm \hspace{3pt}1 - e^{-S \times \frac{t}{1-RR}} \\
    \rm P_{ISDA} &= \rm \hspace{3pt}1 - e^{-600 \times \frac{1}{1 - 0.40}}\\
    \rm P_{ISDA} &= \rm \hspace{3pt}1 - .90484 \\
    \rm P_{ISDA} &= \rm \hspace{3pt}.09516
\end{align}

Now that we are familiar with $P_{constant}$ and $P_{ISDA}$, let's see how they compare to other types of risk-neutral probabilities of default. The table below shows the annual, cumulative, constant, and ISDA probabilities of default for the same sample CDS contract used in \ref{sec:nonconstprob} and \ref{table.nonconstprob}:
...
\begin{tabular}{llllll}
  \hline
Date & $P$ & $P_{cum}$ & $P_{constant}$ & $P_{ISDA}$ \\ 
  \hline
  Jan 1,  1\% & 6\% & 5.8\% & 9.5\% \\ 
  Mar 31, 1\% & 5.75\% & 4.2\% & 6.9\% \\ 
  Jun 30, 1\% & 5.50\% & 2.7\% & 4.4\%\\ 
  Jan 1,  5\% & 5\% & 4.8\% & 7.9\% \\ 
  Mar 31, 5\% & 3.75\% & 2.7\% & 4.5\% \\ 
  Jun 30, 5\% & 2.50\% & 1.2\% & 2.0\% \\ 
  Dec 31, 5\% & 0\% & 0\% & 0\% \\
   \hline
\end{tabular}
}
\caption{This table shows the evolution of the annual, cumulative, constant, and ISDA probabilities of default over a two year period. $P$ changes on January 1 and remains constant for each year because it is the overall probability for one year at a time. $P_{cum}$ decreases proportionately to $P$ for each year. It is the fraction of $P$ that remains for each year, added up. $P_{constant}$ is the single risk-neutral probability that would equate the buyer and seller's cash flows for the remainder of the contract. $P_{ISDA}$ is higher than some of the other probabilities because it accounts for the common ISDA recovery rate of 40\%. The recovery rate lowers the protection payment by eliminating the reference entity's retained assets. A higher probability of default compensates for the reduced notional.}
\label{table.allprob}
\end{table}

We can see that the annual $P$ stays the same within each year because it is the overall risk-neutral probability for a year at a time. $P_{cum}$ decreases towards 0 as it becomes clearer that the reference entity will not default during the contract. It is the sum of the remaining portions of each period. The constant probability of default is what the the probability of default would be if $P_1$ and $P_2$ were equal. Because $P_{ISDA}$ accounts for recovery rate, the protection buyer receives less money if a credit event occurs. Therefore, the ISDA probability tends to be higher than other probabilities.
...
%%Eliminating calculus and focusing on concepts
Having understood the basics of CDS, we can now consider the N-period case, which is more realistic than one- and two-period cases. Suppose the protection buyer now buys a CDS from Citigroup with duration of 5 years. Here, the duration of CDS is also called the \textbf{tenor}, so we can say the contract has a tenor of five years. And there are still four payment dates in one year, but different from the four dates assumed in Section \ref{sec:OnePeriod}. In reality, the four payment dates are Mar. 20, Jun. 20, Sept. 20, and Dec. 20. These dates can be also called \textbf{roll dates} or \textbf{IMM dates}. ISDA specified these four roll dates as trading conventions. Assume the protection buyer buys the CDS on Jun. 20, 2014. After the five year tenor, the CDS protection ends (or \textbf{matures}) on Jun. 20, 2019. The date on which CDS protection ends is known as the \textbf{maturity date} or the \textbf{end date}. Here, we picked the CDS trading date to be Jun. 20 to avoid complications such as upfront payments and \textbf{accrual amount}, which we will talk more about later.

Since there are four payment dates in one year and the tenor is five years, this is a 20-period example of CDS.
Although the number of periods has increased dramatically from two to twenty, the central mathematics from our simpler examples remain unchanged:
...
The first part of the cashflows consist of the coupon payments. The coupons are added up for each period of the CDS, so we would add up 20 coupon payments for a 20-period CDS. But these payments are all made at different times, so they are not worth the same amount. As the market changes, interest rates change and the coupon payments get discounted. With many coupon payments, we must continually compound the interest. This means that successive interest rates build on each other. For example, imagine that the third coupon payment gets discounted from \$1,000 to \$800. due to interest. The interest rate for the fourth coupon payment would be taken away from the previous coupon's worth of \$800 and not the coupon's initial value of \$1,000. This compounding of interest applies to N-period CDS contracts whether the interest rate is constant or non-constant. Remember real-world interest rates are always changing. The process of discounting future payments to find their worth on day one of the CDS is known as finding the \textbf{present value}. It is also important to note that in the event of default, the protection buyer no longer has to make coupon payments. If the reference entity defaults in between roll dates, the protection buyer will just have to pay an accrued amount for the time that has passed from the previous roll date until the default.
The next part of a CDS cashflow equation is the protection leg. To estimate the protection payment, we must multiply the notional amount by the risk-neutral probability of default. Recall that this probability is a mathematical construct used for estimating CDS payments in cash flow equations. It highly unlikely and not necessary for the risk neutral $P$ to match up with the true probability of default. In fact, since investors cannot see into the reference entity's future it is impossible to know the true probability of default at any given time during the CDS. The risk neutral $P$ also does not need to have any correlation with either party's estimate of $P$. The notional amount is the amount on which the CDS buyer is purchasing protection. The notional represents what the buyer would receive in the event of default if recovery rates did not exist. However, a reference entity does not lose everything when it is unable to repay bondholder. The assets that the reference entity retains after default are known as the recovery rate. The assumed recovery rate by the ISDA Standard Model is 40\%. Our cashflows must take recovery rate into account. Therefore, we must multiply the product of the notional amount and the probability of default by $1 - RR$. This value is known as the Loss Given Default, or LGD, and it represents the amount that the reference entity loses after defaulting on its bonds. Additionally, we must remember that the protection payment is most frequently made sometime in the middle of the CDS contract. Therefore, the final step in determining the protection payment is discounting the product of the notional amount, the probability of default, and the LGD by the interest rate at the time of the reference entity's default.
Now that we are familiar with the many complications that can impact a CDS, let's take a look at a sample contract that accounts for them. The table below shows a 8-period CDS contract with non-constant interest and probabilities of default. Additionally, the contract does not begin on a roll date, so we must have an accrued coupon. The spread does not align with the coupon, so we are also dealing with an upfront payment. Though unstated in the table, this CDS has a notional amount of \$100,000, coupon of 1\% and the ISDA recovery rate of 40\%.
...
\begin{tabular}{lllllllll}
  \hline
Date & $CF_H$ & $CF_C$ & $P$ & $P_{cum}$ & Spread & $Upfront$ & $Accrued$ & $Interest$\\ 
  \hline
  Jun 20, 2014 & \$0 & \$0 & 1\% & 6\% & 600 bps & \$0 & \$0 & 0\% \\
  Aug 5, 2014 & -\$9,376 & \$9,376 & 1\% & 5.87\% & 587 bps & \$8,876 & \$500 & 1\% \\
  Dec 20, 2014 & -\$1,000 & \$1,000 & 1\% & 5.50\% & 550 bps & \$0 & \$2,000 & 2\% \\
  Jun 20, 2015 & -\$1,000 & \$1,000 & 5\% & 5\%& 500 bps & \$0 & \$4,000 & 4\% \\ 
  Dec 20, 2015 & -\$1,000 & \$1,000 & 5\% & 2.5\%& 250 bps & \$0 & \$6,000 &  8\% \\ 
  Jun 20, 2016 & -\$1,000 & \$1,000 & 5\% & 0\% & 0 bps & \$0 & \$8,000 & 16\%\\ 
   \hline
\end{tabular}
}
\caption{This table displays a multi-period CDS that starts in between roll dates. Because of this, we have an accrued coupon of \$500. The accrued coupon is half of the \$1,000 coupon because the contract begins halfway between the first and second date. Notice that the further we get into the contract, the higher the accrued coupon gets because more unprotected time goes by the later the contract is entered. On each roll date after Highbridge enters the contract, the have to pay the 1\% coupon to Citigroup. Like previous examples, the first year probability of default is 1\% and the second is 5\%. These values lead to an initial $P_{cum}$ of 6\%. For each year, the cumulative probability of default decreases linearly by the overall $P$ for that respective year. The spread, or cumulative worth of the CDS contract, aligns with $P_{cum}$. Highbridge and Citigroup do not think that a 1\% coupon is fair in this situation given the 600 bp spread. Therefore, they decide on an upfront payment at the start of the contract to equalize expected cashflows. The Interest columns provide sample interest rates with which we can discount the coupon payments to find their present values.}
\label{table.complications}
\end{table}

In the multi-period CDS shown above, the coupon is 1\% of the notional amount of \$100,000. Therefore, Highbridge pays \$1,000 to Citigroup on the roll dates in order to purchase protection. However, this contract is entered halfway in between the June and September roll dates. In order to avoid paying for the time that they were not protected against the reference entity's default, Highbridge receives a \$500 accrued coupon from Citigroup when the contract begins. The accrued coupon would be higher if Highbridge entered the contract later because the later a contract is entered, the more unprotected time has gone by. As in \ref{table.nonconstprob}, the first year probability of default is 1\% and the second year $P$ is 5\%. These probabilities sum to an initial $P_{cum}$ of 6\%. For each year, the cumulative probability of default goes down linearly by the $P$ for the correlating year. $P_{cum}$ matches up with the spread because both values represent the risk of default between a given date during the CDS and the maturity. ISDA says that Highbridge and Citigroup must trade at 1\% or 5\% of the notional amount, and in this case the two parties have chosen 1\%. However, they agree that Highbridge is underpaying for protection. In order to make the expected cashflows equal while remaining within the ISDA boundaries, Highbridge and Citigroup use the ISDA Standard Model to settle on an upfront payment of \$8,876.

\subsubsection{In the Case of a Default}

When a default does occur, the protection seller would owe the protection buyer the notional amount multiplied by the loss given default. It must be noted that the protection is effective for the credit events that have taken place since 60 days \textit{before} the trade date; this date is known as the \textbf{backstop date}. If there is a delay in awareness about a credit event, the new system allows sufficient time for the two participants to discover and process the information. 
...
The alternative to a physical settlement is a \textbf{cash settlement}, in which the seller simply pays the following to the buyer: notional amount $\times$ (1 - recovery rate). ISDA requires an \textbf{auction} style process in which major dealers submit their bids for the value they place on a company's debt.

The following table shows an N-period CDS with a coupon of 1\% and a notional amount of \$100,000. A default occurs in the second year. This example builds off of \ref{table.4} and \ref{table.nonconstprob}, but includes some new aspects such roll dates, default, and accrued coupon. Instead of a physical settlement, this CDS is satisfied by cash settlement where the protection seller pays the buyer after default:
...
\begin{tabular}{llllllll}
  \hline
Date & $CF_H$ & $CF_C$ & $P$ & $P_{cum}$ & Spread & $RR$ & $Accrued$ \\ 
  \hline
  Jun 20, 2014 & -\$1,000 & \$1,000 & 1\% & 6\% & 600 bps & 40\% & \$0 \\ 
  Sep 20, 2014 & -\$1,000 & \$1,000 & 1\% & 5.75\% & 575 bps & 40\% & \$0 \\ 
  Dec 20, 2014 & -\$1,000 & \$1,000 & 1\% & 5.50\% & 550 bps & 40\% & \$0 \\ 
  Mar 20, 2015 & -\$1,000 & \$1,000 & 1\% & 5.25\% & 525 bps & 40\% & \$0 \\ 
  Jun 20, 2015 & -\$1,000 & \$1,000 & 5\% & 5\%& 500 bps & 40\% & \$0 \\ 
  Jul 20, 2015 & \$69,667 & -\$59,667 & 4.625\% & -\$500 & 462.5 bps & 40\% & \$333 \\
   \hline
\end{tabular}
}
\caption{Here we have a sample N-period CDS contract with a default in the second year. The coupon is 1\% of the \$100,000 notional amount, and it is paid on all four roll dates each year. The overall probability for the first year is 1\%, so $P_{cum}$ decreases linearly by 1\% during year one. The overall $P$ for year two is 5\%, leading to and initial cumulative probability of default of 6\%. $P_{cum}$ mirrors the spread because both of these categories represent the risk of default from a specific date until the end of the contract. We use the ISDA estimated recovery rate of 40\%, and this rate remains constant throughout the life of the CDS. After the reference entity defaults, Citigroup, the protection seller, must pay the notional amount after the recovery has been subtracted. This value is know as the loss given default. Finally, since the default happened a third of the way in between roll dates, the protection buyer must pay an accrued coupon of \$333 to end the contract.}
\label{table.nper}
\end{table}

This table displays a 20-period CDS in which the reference entity defaults between the fifth and sixth periods. As in \ref{table.nonconstprob}, the protection buyer, Highbridge, pays a 1\% coupon of \$1,000 for each period that the reference entity does not default. She does not have to pay the coupon after the reference entity defaults. $P$, the yearly overall probability of default is 1\% for the first year and 5\% for the second. $P_{cum}$ decreases linearly per year by the value of $P$ for that respective year. It start at 6 because that is the sum of both values of $P$. The spread decreases in correlation with the cumulative probability of default. Due to the conventions of the ISDA Standard Model, the recovery rate remains at a constant 40\%. This means that ISDA believes that the reference entity will retain 40\% of its assets after default. When the reference entity defaults, the 40\% is taken from the notional amount of \$100,000. And because default occurred one third of the way between roll dates, Highbridge must pay an accrued coupon of \$333 on July 20 that is a third of the original coupon. Subtracting the accrued coupon from the loss given default, we are left with a protection leg payout of \$59,667.
...
The notional, or some portion of the notional must be paid if a \textbf{credit event} occurs. A credit event is something that happens to the reference entity (such as defaulting on bonds) that requires the protection seller to pay the protection buyer. Under SNAC contracts in the US, credit events include filing for bankruptcy and the inability to repay bonds or loans. In Europe under MMR contracts, a restructuring event can occur in addition to the credit events mentioned above. A decrease in coupon price or an extension in the maturity cause a restructuring events, but these events do not happen frequently, so we will mostly just focus on credit events. The International  Swaps and Derivatives Association (ISDA) serves as a governing body for CDS contracts. The ISDA Determination Committee, founded in 2009, decides if a credit event occurs. This single committee ensures that decisions are reached through a consistent process and are not connected to the parties involved with the CDS contract. The committee is made up of 10 sell-side and 5 buy-side firms who vote on the decision, as well as 3 consultative firms and central counterparty observers. All votes and voting dealers are freely and publicly available on the ISDA website for transparency purposes. The Determination Committee members spend an average of one day to reach a decision, and out of more than 900 decisions in the past 3 years, 96\% have been reached unanimously. If the Determination Committee rules that a credit event has happened, they must then figure out whether a CDS auction should be held or the credit event should be settled through cash. (see Toublan, Wopperer 2016 or ISDA 2016 for more on credit events and the Determination Committee)
...
The ISDA protocol, since April 2009, specifies that all premium payments, by default, start on the roll date before the Trade Date (the beginning of the CDS contract). Remember that these quarterly roll dates fall on the 20th of March, June, and September. So if the Trade Date is June 24, 2014, the \textbf{Accrual Begin Date} is, by default, June 20, 2014. Now if the Accrual Begin Date is 4 days before the Trade Date, protection buyers would not want (and are not obligated) to pay interest for the 5 days they have not received protection for. Additionally, as time progresses, we get a better idea of whether or not the reference entity will default. Thus, the risk-neutral probability of default changes and the cashflows are no longer equal (usually putting the protection seller at an advantage). An \textbf{accrued coupon} is used to account for the unprotected days and equate the cash flows, allowing for a more realistic CDS contract. The accrued coupon for our example, in which the buyer is unprotected for 5 days out of the first year,can be calculated using the equation below. Remember that the SNAC conventions cause us to use a daycount of 360 days per year.
...
(see Markit, 2010 for more info on the Markit CDS caluclator)
