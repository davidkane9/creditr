...
%% 2. Also as in the property insurance example, we assumed that both parties would only agree to the deal if the expected cash flows were equal.
%% the connection between equal cash flow and no profit is not very clear. need to mention somewhere in the section above that in a deal cash flows between two counterparties sum to zero.
