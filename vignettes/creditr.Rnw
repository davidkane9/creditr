...
\textbf{Cashflows} show the gain or loss on a specific day for each side of the CDS contract. It is important to understand how cashflows differ from Profits \& Losses. Cashflows strictly show the amount of money that changed hands on the exact day that that cash was exchanged. P\&L is more of a theoretical concept, representing when payments are certain and unable to be cancelled out by other payments. Cashflows, on the other hand, are a concrete record of actual payments. 
...
However, the example above still too simple because the probability of default can change within a period. Recall \ref{table.4}. Let's look at the effect of non-constant probability of on the spread of the CDS. Remeber that the annual probability of default, $P$, represents the overall proability of default for on year. The cumulative probability of default, $P_{cum}$, displays the total probability of default from a given date until the end of the contract. For the following table, we are disregarding the effects of interest and recovery rate.
...
\caption{This table shows a CDS contract with a non-constant probability of default. The table also portrays the effect of probability on spread over the life of of this two year, two period CDS. The annual probability of default is 1\% for year one and 5\% year two. The cumulative probability of default decreases linearly by the year as we get closer to the realization that Alcoa will not default. The spread matches up with $P_{cum}$ because the spread is the total worth of the protection from a specific date at the end. Remember that the spread is the most common way that investors compare CDS and the spread is rate at which CDS would be traded in the abscence of fixed coupon rates.}
\label{table.nonconstprob}
\end{table}

\ref{table.nonconstprob} shows a two period CDS over a two year duration. The contract has an annual probability of default is 1\% for year one and 5\% year two. These two numbers result an initial cumulative probability of default of 6\%. $P_{cum}$ decreases linearly by the respective annual probability of default for each year. The spread, or desired payment for protection, aligns with $P_{cum}$. Recall that portfolio managers and investment banks most frequesntly compare CDS using spread.
...
\begin{tabular}{lllllllll}
  \hline
Date & $CF_H (pv)$ & $CF_C (pv)$ & $P$ & $P_{cum}$ & Spread & $Upfront$ & $Accrued$ & $Interest$\\ 
  \hline
