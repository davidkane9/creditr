...
\fontsize{12pt}{14.4} \textbf{Nam Nguyen} \hspace{10pt}
\fontsize{12pt}{14.4} \textbf{Ben Czenkanski} \hspace{10pt} }
...
%% In the simple two-period case section, explore scenarios in which b P??? changes from one year to the next, and in which b i??? changes from one year to the next. Then, explain the relevance of the swap curve (and ISDA assumption of a constant probability of default).
