...
If we do not have all the information in this table the following two equations can be used to calculate annual probability of default, $P$, or the spread, $S$ (which can easily lead us to $P_{cum}$). Let $RR$ be recovery rate and let $t$ be the time in years until maturity. Note that these two equations are the inverses of each other:

\begin{align}
    \rm P &= \rm \hspace{3pt}1 - e^(-S * frac{t}{1-RR}) \\
    \rm S &= \rm \hspace{3pt}ln(1-P)\times \frac{R-1}{t}
\end{align}

These equations allow us to find to more types of probability of default: the constant probability of default and the ISDA probability of default. When we have multiple probabilities of default for multiple periods in a CDS contract, $P_{constant}$ is the risk neutral value where all of the underlying probabilities are equal. If we know the spread, duration, and recovery rate, we can plug in these values to get $P_{constant}$. The ISDA default recovery rate is 40\%. So by plugging in 0.40 for RR, we can find $P_{ISDA}$.

%% Add a table of constant and ISDA once I work out the errors in those calculations.
